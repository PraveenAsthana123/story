...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Each phase has a named owner per dissertation 15-role per-department structure. Per-phase governance decision row fires per audit policy.
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\raggedright\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{2.3cm} p{2.8cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{3.6cm} @{}}
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Each phase has all 8 dimensions specified; per-phase test runs verify the contract. + (governance) + (production-grade discipline).
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Each AI dimension is auditable per phase. Tech stack + Model Context Protocol (MCP) + multi-agent orchestration are cross-cutting: MCP for tool-call audit (Layer 6); multi-agent (council pattern) for diverse explanation + reduced single-model bias..
