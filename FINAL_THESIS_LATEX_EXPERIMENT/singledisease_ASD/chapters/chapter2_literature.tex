...
\subsection*{Chapter 2 --- Three Questions This Chapter Answers}
\addcontentsline{toc}{subsection}{Chapter 2 --- Three Questions This Chapter Answers}
\label{sec:ch2_three_questions}
...
\caption[Chapter 2 three-question landing page]{Chapter 2 three-question landing page.}\label{tab:ch2_three_questions} \\
\toprule
...
\bottomrule
\endlastfoot
1.~What is already known? & AI-assisted ASD screening is technically feasible; explainability, trust, and adoption have each been studied in isolation; governance is discussed in principle.\\
 & \textit{See:}~\autoref{sec:ch2_strengths_limitations} \\
\addlinespace
2.~What is not known? & How governance maturity is operationally measured, and how it links empirically to explainability, trust, and adoption readiness as a single mediation chain.\\
 & \textit{See:}~\autoref{sec:ch2_unresolved_issues} \\
\addlinespace
3.~Why does the gap matter for theory and practice? & Theory needs extension (governance as antecedent); practice needs an operational, measurable framework. The dissertation closes both.\\
 & \textit{See:}~\autoref{sec:ch2_knowledge_contribution_dedicated} \\
\end{xltabular}}
...
\subsection{Literature Review Philosophy}
\label{sec:literature_review_philosophy}

\noindent The purpose of this literature review is not merely to summarize prior research but to critically evaluate the state of knowledge, identify unresolved contradictions, and establish the theoretical and practical justification for the proposed governance-centered adoption framework. Accordingly, emphasis is placed on synthesis rather than description, and on identifying gaps that directly motivate the present study.
...
\subsection{Evolution of AI Research Phases}
\label{sec:evolution_ai_research}

\noindent The evolution of artificial intelligence research can be broadly categorized into three phases. The first phase focused on predictive performance, emphasizing accuracy and classification outcomes. The second phase emphasized explainability and transparency to address concerns regarding black-box decision making. The third and current phase focuses on governance, accountability, trust, and responsible deployment. This progression suggests that the central challenge facing healthcare AI is no longer whether models can predict accurately but whether organizations can deploy such systems responsibly and sustainably.


\subsection{Extending TAM for AI-Assisted Decision Support}
\label{sec:extending_tam_for_ai}

\noindent While TAM provides a robust explanation of technology adoption behavior, it was developed in the context of conventional information systems. AI-assisted decision-support systems introduce additional concerns relating to transparency, accountability, explainability, and governance. Consequently, governance maturity is proposed as an upstream organizational capability that influences stakeholder perceptions before traditional technology acceptance mechanisms are activated. The proposed framework therefore extends TAM rather than replacing it.


\subsection{Governance as the Missing Variable in Responsible AI Adoption}
\label{sec:governance_missing_variable}

\noindent Across the reviewed literature, governance consistently emerges as an implicit concern yet remains insufficiently measured. Researchers frequently discuss explainability, fairness, transparency, accountability, privacy, and trust; however, these concepts are often examined independently. Governance provides the organizational mechanism through which these concepts are coordinated, monitored, and enforced. Consequently, governance maturity is positioned in this dissertation as the foundational capability underlying responsible AI adoption.
...
\subsection{Contradictions in the Adoption Literature}
\label{sec:contradictions_adoption_literature}

\noindent Although numerous studies report high classification accuracy for AI-assisted healthcare applications, findings regarding adoption remain inconsistent. Several studies suggest that improved explainability increases trust and adoption, while others report limited behavioral impact despite substantial technical transparency improvements. Similarly, healthcare organizations frequently acknowledge the potential value of AI systems while simultaneously expressing concerns regarding accountability, governance, and liability. These contradictions suggest that technical explainability alone may be insufficient unless supported by broader governance mechanisms.


\subsection{Why Existing Responsible AI Frameworks Are Insufficient}
\label{sec:why_existing_frameworks_insufficient}

\noindent Existing Responsible AI frameworks provide valuable guidance regarding fairness, transparency, accountability, privacy, and governance. However, most frameworks remain normative in nature and provide limited empirical evidence regarding how governance mechanisms influence trust and adoption outcomes. Furthermore, few frameworks operationalize governance maturity as a measurable organizational construct capable of influencing stakeholder behavior. This limitation motivates the development and evaluation of the proposed RGAIG framework.


\subsection{Five Recurring Limitations Identified in Prior Literature}
\label{sec:five_recurring_limitations}

\noindent The literature review identified five recurring limitations across prior studies. First, governance is frequently discussed conceptually but rarely operationalized empirically. Second, explainability is often evaluated in isolation from organizational governance structures. Third, trust is commonly treated as an outcome variable without examining its governance antecedents. Fourth, adoption studies frequently focus on technology characteristics while neglecting governance maturity. Finally, few studies integrate governance, explainability, trust, and adoption within a single causal framework. Collectively, these limitations define the research gap addressed by the present study.
...
\subsection{Literature-to-Model Justification}
\label{sec:literature_to_model_justification}

\noindent The proposed Governance $\rightarrow$ Explainability $\rightarrow$ Trust $\rightarrow$ Adoption pathway was derived through literature synthesis rather than theoretical assumption alone. Evidence across healthcare AI, technology acceptance, trust in automation, socio-technical systems, and responsible AI governance consistently suggests that stakeholder trust develops when organizational governance mechanisms improve the transparency and explainability of AI-supported decisions. This accumulated evidence provides the foundation for the conceptual model evaluated in the present research.
...
\subsection{Theoretical Contribution of the Literature Review}
\label{sec:ch2_theoretical_contribution}

\noindent The primary theoretical contribution emerging from the literature review is the proposition that governance maturity functions as a structural antecedent to explainability and trust. Existing adoption theories largely assume technology characteristics as primary drivers of acceptance. The present study proposes that organizational governance capabilities influence how stakeholders interpret, trust, and ultimately adopt AI-assisted systems. This proposition forms the theoretical basis for the empirical evaluation conducted in Chapter~3 and Chapter~4.




\subsection{Distinction Table: Research Gaps Summary}
\label{sec:distinction_research_gap}

\noindent The following distinction-level table consolidates the five recurring research gaps identified across the literature into a single examiner-facing reference, mapping each gap to its source pattern and how the present study addresses it.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.7cm} >{\raggedright\arraybackslash}p{3.3cm} L L @{}}
\caption[Research Gaps Summary]{Research Gaps Summary: five recurring gaps in prior literature with study response.}\label{tab:distinction_research_gaps} \\
\toprule
\thead{Gap} & \thead{Description} & \thead{Pattern in Prior Literature} & \thead{How This Study Addresses} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
G1 & Governance operationalization gap & Discussed conceptually; rarely measured & Operationalized as measurable construct via RGAIG maturity scale \\
\addlinespace
G2 & Explainability--governance integration gap & Explainability evaluated in isolation & Examined as mediator in governance$\rightarrow$trust pathway \\
\addlinespace
G3 & Trust antecedent gap & Trust treated as outcome only & Trust positioned as mediator with governance antecedents \\
\addlinespace
G4 & Adoption maturity gap & Adoption studies focus on technology characteristics & Governance maturity introduced as upstream antecedent \\
\addlinespace
G5 & Integrated causal framework gap & Constructs examined independently & Unified governance$\rightarrow$explainability$\rightarrow$trust$\rightarrow$adoption model \\
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} G1--G5 are addressed simultaneously by the proposed RGAIG framework and the structural-equation model evaluated in Chapter~4. No prior study has addressed all five gaps within a unified ASD-focused empirical context.


\subsection{Distinction Table: Theoretical Foundations Mapping}
\label{sec:distinction_theory_mapping}

\noindent The following distinction-level table maps each foundational theory used in the dissertation to its source author, its role in the conceptual model, and the chapter where it is empirically operationalized.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{3.0cm} L >{\raggedright\arraybackslash}p{2.0cm} @{}}
\caption[Theoretical Foundations Mapping]{Theoretical Foundations Mapping: theory $\rightarrow$ source $\rightarrow$ role $\rightarrow$ operationalization chapter.}\label{tab:distinction_theory_mapping} \\
\toprule
\thead{Theory} & \thead{Foundational Source} & \thead{Role in Conceptual Model} & \thead{Operationalized} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Technology Acceptance Model (TAM) & Davis (1989); Venkatesh \& Davis (2000) & Adoption outcome explanation; extended by governance antecedent & Ch.3 + Ch.4 \\
\addlinespace
Trust-in-Automation Theory & Lee \& See (2004) & Trust as mediating mechanism between explainability and adoption & Ch.3 + Ch.4 \\
\addlinespace
Socio-Technical Systems Theory & Cherns (1976); Trist \& Bamforth (1951) & Organizational context for AI deployment; governance as social subsystem & Ch.3 + Ch.5 \\
\addlinespace
Responsible AI Frameworks & Floridi (2019); EU AI Act; NIST AI RMF & Governance, accountability, transparency principles & Ch.3 (RGAIG) \\
\addlinespace
Explainable AI Theory & Lundberg \& Lee (2017, SHAP); Ribeiro et al.\ (2016, LIME) & Explainability operationalization mechanism & Ch.3 + Ch.4 \\
\addlinespace
TOE Framework & Tornatzky \& Fleischer (1990) & Technology--Organization--Environment integration for adoption context & Ch.1 + Ch.3 \\
\addlinespace
RBV (Resource-Based View) & Barney (1991) & Governance capability as strategic resource & Ch.5 \\
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} The proposed RGAIG framework synthesizes TAM, Trust-in-Automation, Socio-Technical Systems, and Responsible AI principles into a governance-mediated adoption model. TAM is extended (not replaced) by incorporating governance maturity as an upstream organizational antecedent.


\subsection{Executive Table: ASD Literature Synthesis}
\label{sec:exec_asd_synthesis}

\noindent The following executive-level table synthesizes the key themes emerging from the ASD literature.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[ASD Literature Synthesis]{ASD Literature Synthesis.}\label{tab:exec_asd_synthesis} \\
\toprule
\thead{Theme} & \thead{Key finding} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Early detection & Improves developmental outcomes \\
\addlinespace
Diagnostic delay (3--4 years average) & Common challenge across pediatric populations \\
\addlinespace
Screening complexity & High variability across instruments and contexts \\
\addlinespace
Behavioural-only diagnosis & Limited objectivity; subject to assessor variation \\
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Author-level literature inventory is preserved in Appendix~B.


\subsection{Executive Table: EEG for ASD Screening --- Benefits}
\label{sec:exec_eeg_synthesis}

\noindent The following executive-level table synthesizes the relevance of EEG modality for ASD screening.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[EEG Modality Relevance]{EEG Modality Relevance.}\label{tab:exec_eeg_synthesis} \\
\toprule
\thead{EEG benefit} & \thead{Relevance to screening} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Non-invasive & Suitable for pediatric and consumer-grade deployment \\
\addlinespace
Objective neurophysiological signal & Reduces assessor subjectivity \\
\addlinespace
Early biomarker indicators & Enables earlier screening opportunities \\
\addlinespace
Cost-effective (\$200--\$2{,}000) & Compatible with consumer wearable devices \\
\end{xltabular}}


\subsection{Executive Table: AI Capabilities for Healthcare Screening}
\label{sec:exec_ai_capability}

\noindent The following executive-level table synthesizes how AI capabilities translate to healthcare value.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[AI Capability $\rightarrow$ Healthcare Value]{AI Capability $\rightarrow$ Healthcare Value.}\label{tab:exec_ai_capability} \\
\toprule
\thead{AI capability} & \thead{Healthcare value} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Classification & Screening triage support \\
\addlinespace
Pattern detection in EEG signals & Earlier objective indicators \\
\addlinespace
Automation of repetitive analysis & Faster initial assessment \\
\addlinespace
Multi-modal integration & Cross-data synthesis (signal + behavioural + clinical) \\
\end{xltabular}}


\subsection{Executive Table: XAI Function Value}
\label{sec:exec_xai_function}

\noindent The following executive-level table synthesizes the value of each XAI function.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[XAI Function and Value]{XAI Function and Value.}\label{tab:exec_xai_function} \\
\toprule
\thead{XAI function} & \thead{Stakeholder value} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Transparency & Clinician understanding of AI decisions \\
\addlinespace
Justification & Trust formation via interpretability \\
\addlinespace
Accountability & Governance and audit traceability \\
\addlinespace
Counterfactual reasoning & Decision-modification understanding \\
\end{xltabular}}


\subsection{Executive Table: Trust Drivers in AI-Assisted Healthcare}
\label{sec:exec_trust_drivers}

\noindent The following executive-level table synthesizes the principal trust drivers identified in the literature.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[Trust Drivers in AI-Assisted Healthcare]{Trust Drivers in AI-Assisted Healthcare.}\label{tab:exec_trust_drivers} \\
\toprule
\thead{Trust driver} & \thead{Impact} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Transparency of AI outputs & Higher trust \\
\addlinespace
Explainability of reasoning & Higher trust \\
\addlinespace
Governance maturity & Higher trust (proposed antecedent) \\
\addlinespace
Accountability and audit & Higher trust through traceability \\
\end{xltabular}}


\subsection{Executive Table: Responsible AI Principles}
\label{sec:exec_responsible_ai}

\noindent The following executive-level table synthesizes the principles common across Responsible AI frameworks (ISO/IEC 42001, NIST AI RMF, EU AI Act).

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[Responsible AI Principles (Synthesized)]{Responsible AI Principles (Synthesized).}\label{tab:exec_responsible_ai} \\
\toprule
\thead{Principle} & \thead{Purpose} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Transparency & Understanding of AI behaviour \\
\addlinespace
Accountability & Clear responsibility for AI decisions \\
\addlinespace
Fairness & Equitable treatment across populations \\
\addlinespace
Privacy & Protection of patient and stakeholder data \\
\addlinespace
Human oversight & Clinical control over AI recommendations \\
\end{xltabular}}


\subsection{Executive Table: Governance as the Missing Variable}
\label{sec:exec_governance_gap}

\noindent The following executive-level table summarizes why governance is identified as the missing variable across the literature.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[Governance Gap Summary]{Governance Gap Summary.}\label{tab:exec_governance_gap} \\
\toprule
\thead{Aspect} & \thead{Status in prior literature} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Governance principles & Well-articulated (ISO, NIST, EU AI Act) \\
\addlinespace
Governance measurement & Limited or absent as construct \\
\addlinespace
Governance--adoption linkage & Weak empirical evidence \\
\addlinespace
Governance maturity as antecedent & Not yet operationalized as measurable construct \\
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} This dissertation addresses all four rows by introducing RGAIG governance maturity as a measurable construct.


\subsection{Executive Table: TAM Extension for AI Era}
\label{sec:exec_tam_extension}

\noindent The following executive-level table summarizes how this dissertation extends the Technology Acceptance Model.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[TAM Extension for AI-Assisted Decision Support]{TAM Extension for AI-Assisted Decision Support.}\label{tab:exec_tam_extension} \\
\toprule
\thead{TAM construct} & \thead{Purpose} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Perceived Usefulness (original) & Adoption driver \\
\addlinespace
Perceived Ease of Use (original) & Adoption driver \\
\addlinespace
Governance Maturity (extension) & Upstream antecedent shaping explainability + trust \\
\addlinespace
Explainability (extension) & Mediator between governance and trust \\
\addlinespace
Trust (extension) & Mediator between explainability and adoption \\
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} TAM is extended, not replaced. See Section~\ref{sec:extending_tam_for_ai} for the theoretical justification.


\subsection{Executive Table: Literature Synthesis (Known vs.\ Unknown)}
\label{sec:exec_lit_synthesis_table}

\noindent The following executive-level table summarizes what the literature knows and what remains unknown across each construct.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[Literature Synthesis: Known vs.\ Unknown]{Literature Synthesis: Known vs.\ Unknown.}\label{tab:exec_lit_synthesis_table} \\
\toprule
\thead{Theme} & \thead{What we know  /  What we do not know} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Governance & Principles known; measurement gap \\
\addlinespace
Explainability & Methods known (SHAP, LIME, RAG); adoption link gap \\
\addlinespace
Trust & Drivers identified; governance antecedent gap \\
\addlinespace
Adoption & TAM well-validated; governance-mediated pathway gap \\
\end{xltabular}}


\subsection{Executive Table: Gap-to-Hypothesis Mapping}
\label{sec:exec_gap_to_hypothesis}

\noindent The following executive-level table maps each identified research gap to its corresponding hypothesis.

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{6cm} L @{}}
\caption[Gap-to-Hypothesis Mapping]{Gap-to-Hypothesis Mapping.}\label{tab:exec_gap_to_hypothesis} \\
\toprule
\thead{Gap} & \thead{Hypothesis addressed} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
G1: Governance not measured & H1: Governance$\rightarrow$Explainability \\
\addlinespace
G2: Explainability--adoption link unclear & H3: Explainability$\rightarrow$Trust \\
\addlinespace
G3: Trust mechanism unclear & H4: Trust$\rightarrow$Adoption \\
\addlinespace
G4: Adoption pathway unclear & H5+H6: Mediation \\
\addlinespace
G5: Integrated framework absent & RGAIG validation (joint H1--H6) \\
\end{xltabular}}


\subsection{Chapter 2 DBA Template Compliance Additions}
\label{sec:ch2_template_compliance}

\noindent The following dedicated subsections close the remaining DBA template-compliance gaps flagged in the Chapter~2 audit: Strengths and Limitations of Prior Research, Unresolved Issues, Contribution to Knowledge, and Justification for the Research Approach.

\subsubsection{Strengths and Limitations of Prior Research}
\label{sec:ch2_strengths_limitations}

\noindent The literature reviewed in this chapter is strong on technical performance and principle articulation but weak on the operational measurement of governance and on integrated adoption modelling. The matrix below makes the strength--limitation balance explicit per research stream so that the research gap is grounded in evidence rather than rhetoric.

{\tablestripes\tablefontsize
\needspace{18\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{5.5cm} L @{}}
\caption[Strengths and limitations of prior research, by stream]{Strengths and limitations of prior research, by stream.}\label{tab:ch2_strengths_limitations} \\
\toprule
\thead{Stream} & \thead{Strength} & \thead{Limitation} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
ASD AI screening & Strong feasibility evidence + high reported accuracy on benchmarks. & Adoption conditions and governance largely unaddressed. \\
\addlinespace
Explainable AI & Mature catalogue of techniques (SHAP, LIME, attention). & Technology-centric; weak link to organisational governance. \\
\addlinespace
Governance studies & Rich principles, frameworks, and regulatory guidance. & Governance rarely operationalised as a measurable construct. \\
\addlinespace
Trust research & Validated scales for trust in technology and automation. & Trust often treated as outcome; antecedents under-specified for AI. \\
\addlinespace
Adoption research (TAM family) & Robust statistical track record for predicting adoption intent. & Governance not included as an antecedent in extant TAM extensions. \\
\addlinespace
Responsible AI & Principle-level consensus across industry and academia. & Principle-to-practice gap; no widely adopted operational maturity model. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} The right-most column defines the research opportunity the dissertation closes.


\subsubsection{Unresolved Issues in the Literature}
\label{sec:ch2_unresolved_issues}

\noindent Beyond the strength--limitation balance, several specific issues are explicitly unresolved in the literature and form the active questions this dissertation engages.

{\tablestripes\tablefontsize
\needspace{16\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Unresolved issues in the AI-assisted healthcare adoption literature]{Unresolved issues in the AI-assisted healthcare adoption literature.}\label{tab:ch2_unresolved_issues} \\
\toprule
\thead{Unresolved issue} & \thead{Reason it remains open} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
How should governance maturity be measured rather than declared? & No widely adopted operational maturity scale linked to adoption outcomes. \\
\addlinespace
Is explainability a direct trust driver or a mediator? & Mixed empirical results; few studies test mediation explicitly. \\
\addlinespace
How does governance interact with trust formation in clinical contexts? & Most studies isolate either governance or trust, rarely both. \\
\addlinespace
Which adoption antecedents matter most for B2C-primary care contexts? & Most adoption research is enterprise-focused; B2C-primary care under-studied. \\
\addlinespace
What governance maturity threshold is sufficient for deployment? & Without a measurable scale, thresholds cannot be defined. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} Each unresolved issue is addressed by one of the hypotheses H1--H5 tested in Chapter~4.


\subsubsection{Contribution to Existing Knowledge}
\label{sec:ch2_knowledge_contribution_dedicated}

\noindent The dissertation contributes new knowledge in five specific places where the literature reviewed in this chapter is currently silent or contested. The full knowledge contribution matrix is in Appendix~V.5; the chapter-level summary is below.

{\tablestripes\tablefontsize
\needspace{16\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Chapter 2 summary --- contribution to existing knowledge]{Chapter 2 summary --- contribution to existing knowledge.}\label{tab:ch2_knowledge_contribution} \\
\toprule
\thead{Existing knowledge} & \thead{New contribution} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Adoption models without governance & Adoption model extended with governance maturity as antecedent (operationalised). \\
\addlinespace
Trust models without governance antecedent & Empirical evidence that governance is a structural antecedent of trust. \\
\addlinespace
Explainable AI studied in isolation & Explainability validated as a mediator linking governance to trust. \\
\addlinespace
Responsible AI as principles & Operational RGAIG framework with measurable constructs. \\
\addlinespace
Governance maturity discussed conceptually & Governance maturity validated as a measurable, empirically meaningful construct. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} The knowledge gaps this contribution closes are sourced from the strength--limitation matrix above.


\subsubsection{Justification for the Research Approach}
\label{sec:ch2_approach_justification}

\noindent The literature gap is dual: theory needs extension (governance as antecedent) \emph{and} practice needs operationalisation (governance as measurable construct). A purely quantitative approach would close the statistical gap but not the contextual gap; a purely qualitative approach would close the contextual gap but not the statistical gap. The mixed-methods design adopted in Chapter~3 is therefore the minimum-sufficient approach for the dissertation's contribution. The detailed justification follows in Chapter~3.


\subsection{Chapter 2 Signature Diagrams}
\label{sec:ch2_signature_diagrams}

\noindent This subsection consolidates the five highest-value signature diagrams for Chapter~2. They capture the evolution of AI research from accuracy to governance, the trust formation chain, the governance literature trajectory, the theoretical lineage of the RGAIG framework, and the path from literature to the testable conceptual model.


\noindent AI research has migrated through four eras. Each era solved one problem but exposed the next. Recognising this trajectory makes the current focus on governance-centered AI not an arbitrary choice but the inevitable next layer of maturity. Figure~\ref{fig:ch2_ai_research_evolution} captures this flow.
...
\node[font=\figtitlefont, ggunavy] (title) at (0,0.7) {Evolution of AI research --- accuracy to governance};
\node[fbox, below=0.85cm of title] (n0) {Era 1: AI Performance (accuracy benchmarks)};
\node[fbox, below=of n0] (n1) {Era 2: Explainable AI (what the model sees)};
\node[fbox, below=of n1] (n2) {Era 3: Responsible AI (principles and ethics)};
\node[fboxgold, below=of n2] (n3) {Era 4: Governance-Centered AI (operational accountability)};
\draw[farr] (n0) -- (n1);
\draw[farr] (n1) -- (n2);
\draw[farr] (n2) -- (n3);
\end{tikzpicture}
\caption[Evolution of AI research --- accuracy to governance]{Evolution of AI research --- accuracy to governance.}
\label{fig:ch2_ai_research_evolution}
\end{figure}

\vspace{0.3em}\noindent\textit{Note.} The dissertation positions itself in Era~4 --- moving principles into measurable organizational practice.



\noindent Trust is not granted; it is constructed. The literature converges on a sequence in which governance enables transparency, which enables explainability, which becomes trust, which licenses adoption. Figure~\ref{fig:ch2_trust_formation} captures this flow.
...
\node[font=\figtitlefont, ggunavy] (title) at (0,0.7) {Trust formation chain in AI-assisted healthcare};
\node[fbox, below=0.85cm of title] (n0) {Governance (the structural foundation)};
...
\node[fboxgold, below=of n3] (n4) {Adoption (intent to use)};
\draw[farr] (n0) -- (n1);
\draw[farr] (n1) -- (n2);
\draw[farr] (n2) -- (n3);
\draw[farr] (n3) -- (n4);
\end{tikzpicture}
\caption[Trust formation chain in AI-assisted healthcare]{Trust formation chain in AI-assisted healthcare.}
\label{fig:ch2_trust_formation}
\end{figure}

\vspace{0.3em}\noindent\textit{Note.} Each arrow is supported by at least three independent studies in Chapter~2's literature.



\noindent Governance scholarship has matured from compliance checklists to multi-dimensional maturity models. The dissertation builds on the most recent layer, in which governance is measured rather than merely declared. Figure~\ref{fig:ch2_governance_evolution} captures this flow.
...
\node[font=\figtitlefont, ggunavy] (title) at (0,0.7) {Evolution of AI governance literature};
\node[fbox, below=0.85cm of title] (n0) {Compliance (regulatory minima)};
\node[fbox, below=of n0] (n1) {Risk Management (mitigation portfolios)};
\node[fbox, below=of n1] (n2) {Responsible AI (principled deployment)};
\node[fboxgold, below=of n2] (n3) {Governance Maturity (measurable capability)};
\draw[farr] (n0) -- (n1);
\draw[farr] (n1) -- (n2);
\draw[farr] (n2) -- (n3);
\end{tikzpicture}
\caption[Evolution of AI governance literature]{Evolution of AI governance literature.}
\label{fig:ch2_governance_evolution}
\end{figure}

\vspace{0.3em}\noindent\textit{Note.} The RGAIG framework operationalises the right-most layer in the methodology of Chapter~3.



\noindent RGAIG is not a standalone construction; it is the integration of four established theoretical traditions into a single governance-centered adoption model. Figure~\ref{fig:ch2_theory_integration} captures this flow.
...
\node[font=\figtitlefont, ggunavy] (title) at (0,0.7) {Theoretical lineage of the RGAIG framework};
\node[fbox, below=0.85cm of title] (n0) {Technology Acceptance Model (TAM)};
\node[fbox, below=of n0] (n1) {Trust Theory (interpersonal $\rightarrow$ technological)};
\node[fbox, below=of n1] (n2) {Socio-Technical Systems (joint optimisation)};
\node[fbox, below=of n2] (n3) {Responsible AI (operational principles)};
\node[fboxgold, below=of n3] (n4) {RGAIG Framework (synthesis)};
\draw[farr] (n0) -- (n1);
\draw[farr] (n1) -- (n2);
\draw[farr] (n2) -- (n3);
\draw[farr] (n3) -- (n4);
\end{tikzpicture}
\caption[Theoretical lineage of the RGAIG framework]{Theoretical lineage of the RGAIG framework.}
\label{fig:ch2_theory_integration}
\end{figure}

\vspace{0.3em}\noindent\textit{Note.} The framework's novelty is the synthesis, not any single component.



\noindent The path from literature review to a testable conceptual model has five stages. The figure makes each stage and its output visible so an examiner can trace any model claim back to its source. Figure~\ref{fig:ch2_lit_to_model} captures this flow.
...
\node[font=\figtitlefont, ggunavy] (title) at (0,0.7) {Literature-to-model pathway};
\node[fbox, below=0.85cm of title] (n0) {Literature Review (evidence base)};
\node[fbox, below=of n0] (n1) {Gap Identification (what is missing)};
\node[fbox, below=of n1] (n2) {Theory Synthesis (integrating traditions)};
\node[fbox, below=of n2] (n3) {Conceptual Framework (RGAIG)};
\node[fbox, below=of n3] (n4) {Hypotheses (H1--H5)};
\draw[farr] (n0) -- (n1);
\draw[farr] (n1) -- (n2);
\draw[farr] (n2) -- (n3);
\draw[farr] (n3) -- (n4);
\end{tikzpicture}
\caption[Literature-to-model pathway]{Literature-to-model pathway.}
\label{fig:ch2_lit_to_model}
\end{figure}

\vspace{0.3em}\noindent\textit{Note.} Hypotheses H1--H5 are introduced in Chapter~3 and tested in Chapter~4.



\subsection{Chapter 2 Examiner Summary}
\label{sec:ch2_examiner_summary}

\noindent The literature review demonstrates that substantial progress has been achieved in AI-assisted ASD screening, explainable AI, and healthcare AI deployment. However, existing research provides limited evidence regarding how governance maturity influences stakeholder trust and adoption outcomes. This unresolved issue constitutes the central gap addressed by the present dissertation and directly motivates the proposed RGAIG framework and associated hypotheses.
