...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-step output saved with reproducible random seed; per-step audit row written per dissertation governance policy.


\clearpage
\paragraph[Primary Data To-Do List]{Primary Data --- Active To-Do.}
...
\clearpage
\paragraph[Primary Data Database Design]{Primary Data Database Design ---.}
...
\clearpage
\paragraph[Secondary Data DB Schema]{Secondary Data DB Schema ---.}
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Engine: PostgreSQL 16 + MinIO / S3 for blob; alternative SQLite + local disk for development. Migrations via Alembic; per-migration test verifies schema integrity.


\clearpage
\paragraph[IV DV Hypothesis Matrix]{IV / DV / Hypothesis.}


\noindent This table lists each \textit{h\#} across iv, dv, expected, and 1 more dimension, so examiners can trace what was assessed and what evidence supports each row.
...
\clearpage
\paragraph[Primary Data Preprocess Deep Dive]{Primary Data Preprocess Deep Dive.}
...
\clearpage
\paragraph[Primary Data EDA]{Primary Data EDA --- Per-Construct.}
...
\clearpage
\paragraph[Primary Data Feature Engineering]{Primary Data Feature Engineering ---.}
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} All derived features fully documented in \texttt{features/\allowbreak primary\_\allowbreak feature\_\allowbreak catalog.md}. Per-feature formula reproducible; per-feature test verifies formula consistency.


\clearpage
\paragraph[Primary Data Normalization]{Primary Data Normalization --- Method.}
...
per-normalisation test & Drill: scaler-fit on train fold; verify mean $\approx 0$, SD $\approx 1$ on train; verify no fit on test \\
...
\clearpage
\paragraph[Primary Data Bias Inventory]{Primary Data Bias Inventory ---.}
...
\clearpage
\paragraph[Balanced vs Unbalanced Data Handling]{Balanced vs Unbalanced Data Handling.}
...
\clearpage
\paragraph[Semi-Structured Data Handling]{Semi-Structured Data Handling --- PD-2.}
...
\clearpage
\paragraph[1D 2D Conversion Per Phase]{1D / 2D Conversion Detail.}


\noindent This table lists each \textit{phase} across source dim, conversion method, target dim, and 1 more dimension, so examiners can trace what was assessed and what evidence supports each row.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-phase conversion explicit; per-conversion test verifies dimension transformation correctness. Conversion choices align with downstream model input shape requirements.
