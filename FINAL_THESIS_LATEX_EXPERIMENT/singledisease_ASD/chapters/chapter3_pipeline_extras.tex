...
\noindent\emph{Twenty-eight-table consolidated extras: per-stage detail-flow pages (preprocessing sub-steps, conversion, Fourier, SWT, healthy vs ASD comparison, EDA, graph projection, image conversion), per-stage governance pages (tool/technique, explainability, responsibility, secure AI, governance), model + evaluation + benchmarking pages, analysis-type list pages (statistical / sensitivity / subjective), and AS-IS vs TO-BE process pages (with Emotiv + IoT + Gateway). Each page is a single-page dedicated reference per formatting policy.}


\clearpage
\paragraph[Preprocessing Detail Flow]{Preprocessing Detail Flow --- Sub-Steps.}
...
\noindent Table~\ref{tab:preproc_detail} presents preprocessing detail flow, covering Aspect and Specification.
...
\clearpage
\paragraph[Conversion Flow]{Conversion Flow --- EDF /.}
...
\noindent Table~\ref{tab:conversion_detail} presents conversion flow, covering Aspect and Specification.
...
\clearpage
\paragraph[Fourier Transform Detail]{Fourier Transform Detail --- FFT.}
...
\noindent Table~\ref{tab:fft_detail} presents fourier transform detail, covering Aspect and Specification.
...
\clearpage
\paragraph[SWT Flow]{SWT (Stationary Wavelet.}
...
\noindent Table~\ref{tab:swt_detail} presents swt flow, covering Aspect and Specification.
...
\noindent Table~\ref{tab:healthy_vs_asd_window} presents healthy vs asd data window, covering Feature, Healthy Control, ASD-positive and Difference + interpretation.
...
\clearpage
\paragraph[EDA (Exploratory Data Analysis)]{EDA --- Exploratory Data Analysis.}
...
\noindent Table~\ref{tab:eda_plan} presents eda (exploratory data analysis), covering Aspect and Specification.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} EDA runs before any inferential model fitting; findings inform feature selection (Step 13) and feature engineering choices. Per-step before/after EDA snapshots saved per Before/After Preprocessing Visualization policy.


\clearpage
\paragraph[Graph Projection]{Graph Projection ---.}
...
\noindent Table~\ref{tab:graph_projection} presents graph projection, covering Aspect and Specification.
...
\clearpage
\paragraph[Image Conversion (EEG -> Image)]{Image Conversion --- EEG 1D.}
...
\noindent Table~\ref{tab:image_conversion} presents image conversion (eeg -> image), covering Aspect and Specification.
...
\clearpage
\paragraph[Tool + Technique per Stage]{Tool + Technique Used at.}
...
\noindent Table~\ref{tab:tools_per_stage} presents tool + technique per stage, covering Aspect and Specification.
...
\clearpage
\paragraph[Explainability per Stage]{Explainability Contract at.}
...
\noindent Table~\ref{tab:xai_per_stage} presents explainability per stage, covering Aspect and Specification.
...
\clearpage
\paragraph[Responsibility (RAI) per Stage]{Responsible AI (RAI) Contract at.}
...
\noindent Table~\ref{tab:rai_per_stage} presents responsibility (rai) per stage, covering Aspect and Specification.
...
\clearpage
\paragraph[Secure AI per Stage]{Secure AI Controls at Each.}
...
\noindent Table~\ref{tab:secure_ai_per_stage} presents secure ai per stage, covering Aspect and Specification.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Secure-AI per stage closes the OWASP Top 10 + AI-specific threat surfaces (A11 prompt injection, A12 insecure output, A13 data poisoning, A14 model theft, A15 excessive agency). 4-lens security policy.


\clearpage
\paragraph[Governance per Stage]{Governance Decision Gate at Each.}
...
\noindent Table~\ref{tab:governance_per_stage} presents governance per stage, covering Aspect and Specification.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-stage governance assignment uses the dissertation's 15-role per-department structure (Manager / AI Reviewer / Clinical Reviewer / Data Steward / etc.). Decision audit row per gate fires per governance policy.


\clearpage
\paragraph[Model Architecture Detail]{Model Architecture Detail.}
...
\noindent Table~\ref{tab:model_architecture_detail} presents model architecture detail, covering Aspect and Specification.
...
\clearpage
\paragraph[Accuracy per Model]{Accuracy per Model --- Per-Cohort.}
...
\noindent Table~\ref{tab:accuracy_per_model} presents accuracy per model, covering Aspect and Specification.
...
\clearpage
\paragraph[Benchmarking Comparison]{Benchmarking Comparison --- This Work.}
...
\noindent Table~\ref{tab:benchmarking_comparison} presents benchmarking comparison, covering Aspect and Specification.
...
\clearpage
\paragraph[Statistical Analysis List]{Statistical Analysis List --- Comprehensive.}
...
\noindent Table~\ref{tab:statistical_analysis_list} presents statistical analysis list, covering Aspect and Specification.
...
\clearpage
\paragraph[Sensitivity Analysis List]{Sensitivity Analysis List --- Robustness.}
...
\clearpage
\paragraph[Subjective Analysis List]{Subjective Analysis List --- Qualitative.}
...
\noindent Table~\ref{tab:subjective_analysis_list} presents subjective analysis list, covering Aspect and Specification.
...
\clearpage
\paragraph[Monte Carlo vs Bootstrap Note]{Monte Carlo vs Bootstrap ---.}
...
\clearpage
\paragraph[Secondary Data Capture Process]{Secondary Data Capture Process ---.}
...
\noindent Table~\ref{tab:secondary_data_capture_process} presents secondary data capture process, covering Aspect and Specification.
...
\clearpage
\paragraph[AS-IS Process (Detail)]{AS-IS Process (Detailed Flow) ---.}
...
\noindent Table~\ref{tab:as_is_process_detail} presents as-is process (detail), covering Aspect and Specification.
...
\clearpage
\paragraph[AS-IS Challenges]{AS-IS Challenges --- Friction Points.}
...
\noindent Table~\ref{tab:as_is_challenges} presents as-is challenges, covering Aspect and Specification.
...
\clearpage
\paragraph[TO-BE Process (Emotiv + IoT + Gateway)]{TO-BE Process --- Emotiv EEG.}
