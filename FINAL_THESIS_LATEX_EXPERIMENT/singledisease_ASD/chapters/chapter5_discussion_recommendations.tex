...
\subsection*{Chapter 5 --- Three Questions This Chapter Answers}
\addcontentsline{toc}{subsection}{Chapter 5 --- Three Questions This Chapter Answers}
\label{sec:ch5_three_questions}
...
\caption[Chapter 5 three-question landing page]{Chapter 5 three-question landing page.}\label{tab:ch5_three_questions} \\
\toprule
...
\bottomrule
\endlastfoot
1.~Why does it matter? & Governance maturity is the manageable lever for responsible AI adoption; explainability and trust are downstream consequences, not parallel constructs.\\
 & \textit{See:}~\autoref{sec:ch5_theory_contribution_dedicated} \\
\addlinespace
2.~What is the publishable contribution? & Extended TAM with governance maturity as antecedent (theory) + RGAIG framework (artefact) + four formal propositions P1--P4 ready for publication.\\
 & \textit{See:}~\autoref{sec:ch5_knowledge_contribution_dedicated} \\
\addlinespace
3.~What should happen next? & Adopt the recommendation matrix per audience; multi-site and cross-context validation; extension to GenAI, agentic AI, and physical AI governance (five-phase legacy).\\
 & \textit{See:}~\autoref{sec:ch5_recommendation_matrix_dedicated} \\
\end{xltabular}}
...
{\tablestripes\tablefontsize
\needspace{10\baselineskip}
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{1.0cm} p{4.8cm} >{\centering\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{2.9cm} >{\centering\arraybackslash}p{2.0cm} >{\centering\arraybackslash}p{1.0cm} @{}}
...
{\tablestripes\tablefontsize
\needspace{10\baselineskip}
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{1.0cm} p{5.3cm} >{\raggedright\arraybackslash}p{3.4cm} >{\centering\arraybackslash}p{2.2cm} >{\centering\arraybackslash}p{2.0cm} @{}}
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\raggedright\arraybackslash}p{1.0cm} p{1.0cm} p{2.4cm} p{4.5cm} p{2.6cm} p{2.2cm} @{}}
...
{\tablestripes\tablefontsize
\needspace{10\baselineskip}
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{1.0cm} p{9.5cm} >{\raggedright\arraybackslash}p{1.5cm} @{}}
...
{\tablestripes\tablefontsize
\needspace{10\baselineskip}
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{1.0cm} >{\raggedright\arraybackslash}p{4.8cm} >{\centering\arraybackslash}p{1.0cm} >{\centering\arraybackslash}p{3.3cm} >{\centering\arraybackslash}p{2.6cm} >{\centering\arraybackslash}p{1.0cm} @{}}
...
{\tablestripes\tablefontsize
\needspace{10\baselineskip}
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{3.0cm} p{4.5cm} >{\centering\arraybackslash}p{1.0cm} >{\centering\arraybackslash}p{2.9cm} >{\centering\arraybackslash}p{2.5cm} @{}}
...
\subsection{Chapter 5 DBA Template Compliance Additions}
\label{sec:ch5_template_compliance}

\noindent The following dedicated subsections close the remaining DBA discussion-and-contributions template gaps flagged in the Chapter~5 audit: Theory Contribution, Knowledge Contribution, Literature Comparison, Recommendation Matrix, Managerial Implications, Policy Implications, and Societal Contribution.

\subsubsection{Theory Contribution --- Dedicated Section}
\label{sec:ch5_theory_contribution_dedicated}

\noindent The dissertation extends the Technology Acceptance Model with governance maturity as an antecedent that operates through explainability and trust to influence adoption readiness. The extension is testable, falsifiable, and recurrent: it has been tested in this dissertation (Chapter~4) and can be re-tested in any other domain in which governance, explainability, trust, and adoption are jointly relevant. This is the publishable theoretical contribution.

{\tablestripes\tablefontsize
\needspace{12\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Theory contribution --- existing theory and present extension]{Theory contribution --- existing theory and present extension.}\label{tab:ch5_theory_contribution_dedicated} \\
\toprule
\thead{Existing theory} & \thead{Present extension} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Technology Acceptance Model (TAM) & Governance maturity added as antecedent, operating through explainability and trust. \\
\addlinespace
Trust theory & Governance repositioned as a structural antecedent of trust, not as a downstream outcome. \\
\addlinespace
Responsible AI principles & Operationalised into measurable constructs with empirical validation in Chapter~4. \\
\addlinespace
Socio-technical systems & Empirical evidence that joint optimisation requires explicit governance measurement. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} The four formal propositions P1--P4 expressing this contribution are in Appendix~V.4.


\subsubsection{Contribution to Knowledge --- Dedicated Section}
\label{sec:ch5_knowledge_contribution_dedicated}

\noindent New knowledge is added at five specific places where the literature reviewed in Chapter~2 is silent or contested. The summary below restates the contribution in chapter-level form; the full matrix is in Appendix~V.5.

{\tablestripes\tablefontsize
\needspace{12\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Knowledge contribution summary --- chapter-level restatement]{Knowledge contribution summary --- chapter-level restatement.}\label{tab:ch5_knowledge_contribution_dedicated} \\
\toprule
\thead{Knowledge gap} & \thead{New knowledge from this dissertation} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Governance not operationalised as a measured construct & Governance Maturity validated as a measurable antecedent of explainability and trust. \\
\addlinespace
Explainability disconnected from governance & Explainability empirically linked to governance maturity and to trust. \\
\addlinespace
Trust formation under-theorised for AI & Trust validated as the mediating mechanism between explainability and adoption. \\
\addlinespace
Adoption antecedents incomplete for healthcare AI & Adoption readiness empirically explained by the governance-mediated chain. \\
\addlinespace
Responsible AI principles without operational model & RGAIG provides the empirically grounded operational model. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} Each row is the closing of a specific gap identified in Chapter~2.


\subsubsection{Comparison with Prior Literature}
\label{sec:ch5_literature_comparison}

\noindent Each headline finding is positioned against the prior literature. Where findings agree, the dissertation strengthens an existing consensus; where findings differ, the dissertation flags the boundary condition that explains the difference. This comparison is what an examiner expects in the discussion chapter and is one of the strongest signals of a doctoral-level contribution.

{\tablestripes\tablefontsize
\needspace{12\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Findings versus prior literature --- agreement and difference]{Findings versus prior literature --- agreement and difference.}\label{tab:ch5_literature_comparison} \\
\toprule
\thead{Finding} & \thead{Literature comparison} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Governance Maturity is a measurable construct that predicts perception outcomes. & Extends governance literature beyond principles; agrees with maturity-model traditions but adds adoption predictors. \\
\addlinespace
Explainability mediates governance into trust. & Agrees with explainability-trust literature on direction; adds governance as the antecedent rarely studied. \\
\addlinespace
Trust mediates explainability into adoption. & Agrees with TAM-trust literature; adds the upstream governance-explainability link. \\
\addlinespace
Mediated path beats direct path. & Resolves an open debate by providing empirical evidence for the indirect dominance hypothesis. \\
\addlinespace
RGAIG framework is operationally valid. & Adds an integrated, measurable framework where prior responsible AI work stopped at principles. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} Comparison evidence is summarised in the Claim--Evidence Matrix (Appendix~V.7).


\subsubsection{Recommendation Matrix --- Audience-Specific Actions}
\label{sec:ch5_recommendation_matrix_dedicated}

\noindent The recommendation matrix translates findings into actions per audience. It is one of the strongest distinction-level artefacts in a DBA dissertation because it bridges contribution and practical impact.

{\tablestripes\tablefontsize
\needspace{18\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Recommendation matrix --- audience-specific actions]{Recommendation matrix --- audience-specific actions.}\label{tab:ch5_recommendation_matrix} \\
\toprule
...
\addlinespace
C-suite executives & Treat governance as a strategic adoption lever, not a post-deployment audit task. \\
\addlinespace
AI/ML teams & Build explainability into the model lifecycle as a governance requirement. \\
\addlinespace
Regulators & Adopt measurable governance criteria in healthcare AI policy guidance. \\
\addlinespace
Clinician leadership & Embed explainability literacy into clinical AI training curricula. \\
\addlinespace
Researchers & Treat governance maturity as a measurable antecedent in future adoption studies. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} Cross-referenced with the implementation roadmap in Chapter~5's organisational adoption discussion.


\subsubsection{Managerial Implications}
\label{sec:ch5_managerial_implications}

\noindent The managerial implications are framed as decisions that can be made tomorrow on the basis of the dissertation's findings.

{\tablestripes\tablefontsize
\needspace{12\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Managerial implications of the findings (template compliance)]{Managerial implications of the findings (template-compliance summary).}\label{tab:ch5_managerial_implications_compliance} \\
\toprule
\thead{Area} & \thead{Implication for management} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Governance & Governance maturity is the strategic lever; explainability and trust are downstream consequences. \\
\addlinespace
Explainability & Treat explainability as a governance deliverable, not as a feature add-on. \\
\addlinespace
Trust & Trust cannot be engineered directly; it follows from explainability that follows from governance. \\
\addlinespace
Adoption & Adoption resistance is often a governance gap mis-diagnosed as a trust gap. \\
\addlinespace
Organisational design & Establish a governance owner with measurable maturity-stage objectives before launching clinical AI services. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} Each implication is paired with a recommendation in the matrix above.


\subsubsection{Policy Implications}
\label{sec:ch5_policy_implications}

\noindent The dissertation has direct policy implications because it converts responsible AI principles into measurable governance criteria suitable for regulatory guidance.

{\tablestripes\tablefontsize
\needspace{18\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Policy implications of the findings]{Policy implications of the findings.}\label{tab:ch5_policy_implications} \\
\toprule
\thead{Policy area} & \thead{Implication for policy} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Governance & Regulatory guidance should mandate governance maturity assessment for high-stakes AI. \\
\addlinespace
Transparency & Set transparency requirements at the governance layer, not only at the model layer. \\
\addlinespace
Human oversight & Codify human-in-the-loop requirements as part of governance maturity, not as a separate clause. \\
\addlinespace
Auditability & Require audit-ready evidence trails for AI decisions in clinical settings. \\
\addlinespace
Explainability & Lay-readable explanations should be a governance requirement, not an optional product feature. \\
\addlinespace
Responsible AI & Operational governance criteria (RGAIG-style) should supplement principle statements in regulatory guidance. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} Policy implications are restated in Appendix~V.14 with the managerial and societal levels.


\subsubsection{Societal Contribution}
\label{sec:ch5_societal_contribution}

\noindent The societal contribution is the broader public-trust case for responsible healthcare AI. The matrix below frames the contribution in terms an external policy audience or general public can engage with.

{\tablestripes\tablefontsize
\needspace{12\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{5cm} L @{}}
\caption[Societal contribution --- public-facing framing]{Societal contribution --- public-facing framing.}\label{tab:ch5_societal_contribution} \\
\toprule
\thead{Area} & \thead{Societal impact} \\
\midrule
\endfirsthead
\endhead
\bottomrule
\endlastfoot
Healthcare equity & Governance maturity reduces the risk of inequitable AI deployment across populations. \\
\addlinespace
Public trust in AI & Empirical evidence linking governance to trust strengthens the public-trust case for responsible AI. \\
\addlinespace
Earlier ASD identification & B2C-primary, clinician-supported screening can shorten the diagnostic waiting period. \\
\addlinespace
Responsible AI ecosystem & The framework is generalisable beyond healthcare AI to GenAI, agentic AI, and physical AI contexts. \\
\addlinespace
Educational impact & Explainability literacy is positioned as a competency, not as a technical novelty. \\
\end{xltabular}}

\vspace{0.3em}\noindent\textit{Note.} The societal contribution is the entry point of the five-phase research legacy in Appendix~V.16.
