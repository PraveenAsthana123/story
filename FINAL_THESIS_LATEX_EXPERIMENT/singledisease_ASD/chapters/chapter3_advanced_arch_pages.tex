...
\clearpage
\paragraph[Pipeline Types]{Pipeline Types --- Training / Inference / Fallback / Vector DB / Output Eval.}
...
\noindent Table~\ref{tab:pipeline_types} presents pipeline types, covering Pipeline, Trigger, Latency target, and 2 more.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-pipeline SLO monitored continuously; per-SLO miss triggers RCA + corrective-action per §38.3 governance policy.


\clearpage
\paragraph[Vector DB Qdrant]{Vector DB (Qdrant) --- Configuration + Schema.}
...
\noindent Table~\ref{tab:vector_db_qdrant} presents vector db qdrant, covering Aspect and Specification.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Qdrant chosen for production scale (Rust performance + persistent index); ChromaDB for local development. Per-collection drill verifies recall$@5$ on held-out QA set before promotion.


\clearpage
\paragraph[Graph DB]{Graph DB Design --- Neo4j Schema for Knowledge Graph.}


\noindent This table lists each \textit{aspect} with its specification, so examiners can trace what was assessed and what evidence supports each row.


\noindent Table~\ref{tab:graph_db_design} presents graph db, covering Aspect and Specification.
...
\clearpage
\paragraph[Cron LLM Cache]{Cron Jobs + LLM Model Registry + Cache DB.}


\noindent This table lists each \textit{aspect} with its specification, so examiners can trace what was assessed and what evidence supports each row.


\noindent Table~\ref{tab:cron_llm_cache} presents cron llm cache, covering Aspect and Specification.
...
\clearpage
\paragraph[PII Index Retrieval Chunking]{PII + Index + Pre-Retrieval + Post-Retrieval + Chunking Deep Dive.}
...
\noindent Table~\ref{tab:pii_index_retrieval_chunking} presents pii index retrieval chunking, covering Technique, Purpose, Implementation and Validation.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-technique drill verifies implementation invariant; chunking strategy + embedding model + reranker pinned in MLflow registry for reproducibility.


\clearpage
\paragraph[Output Evaluation Ragas]{Output Evaluation --- Ragas Metrics + Custom Checks.}


\noindent This table lists each \textit{aspect} with its specification, so examiners can trace what was assessed and what evidence supports each row.


\noindent Table~\ref{tab:output_eval_ragas} presents output evaluation ragas, covering Aspect and Specification.
...
\clearpage
\paragraph[Accuracy Benchmarking Business Value]{Accuracy + Benchmarking + Business Value --- Consolidated Reference.}


\noindent This table lists each \textit{aspect} with its specification, so examiners can trace what was assessed and what evidence supports each row.


\noindent Table~\ref{tab:accuracy_bench_business} presents accuracy benchmarking business value, covering Aspect and Specification.
...
\clearpage
\paragraph[RGAIG Per Phase]{RGAIG Framework Mapping Per Pipeline Phase.}
...
\noindent Table~\ref{tab:rgaig_per_phase} presents rgaig per phase, covering Phase, L1 Capture, L2 Cloud, and 5 more.
...
\clearpage
\paragraph[SMART Per Phase]{SMART(-E) Framework Per Pipeline Phase.}
...
\noindent Table~\ref{tab:smart_per_phase} presents smart per phase, covering Phase, Specific, Measurable, and 3 more.
...
\clearpage
\paragraph[Per-Phase Quality Summary]{Per-Phase Quality Summary --- Input / Process / Output / KPI / XAI / RAI / Secure AI / HITL.}
...
\noindent Table~\ref{tab:per_phase_quality_summary} presents per-phase quality summary, covering Phase, Input, Process, and 3 more.
...
\clearpage
\paragraph[Master DB Schema]{Master DB Schema --- Master / Transaction / Org / Condition / Service / Product / Usage / Consumption.}
...
\clearpage
\paragraph[Survey + Report Save Schema]{Survey + Report Save Schema --- Per-Survey + Per-Report Persistence.}


\noindent This table lists each \textit{aspect} with its specification, so examiners can trace what was assessed and what evidence supports each row.


\noindent Table~\ref{tab:survey_report_save_schema} presents survey + report save schema, covering Aspect and Specification.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-survey + per-report save policy enforced via DB trigger; per-write audit row written. Daily backup verified; quarterly restore drill.


\clearpage
\paragraph[Data Flow + Reporting Flow]{Data Flow + Reporting Flow --- End-to-End Schema.}


\noindent This table lists each \textit{aspect} with its specification, so examiners can trace what was assessed and what evidence supports each row.


\noindent Table~\ref{tab:data_flow_reporting_flow} presents data flow + reporting flow, covering Aspect and Specification.
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} End-to-end data flow + reporting flow with per-stage latency SLO. Per-stage drill verifies latency target. Aggregate latency dashboard reported per week.
