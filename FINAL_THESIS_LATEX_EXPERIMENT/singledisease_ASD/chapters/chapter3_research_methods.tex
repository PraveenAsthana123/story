...
\noindent Table~\ref{tab:primary_data_reference_catalog} presents primary data stream catalog, covering ID, Stream, N, and 2 more.
...
\noindent Table~\ref{tab:primary_data_outcome_map} presents primary data outcome map, covering Stream, Chapter~4 finding and Section.
...
\noindent Table~\ref{tab:secondary_data_reference_catalog} presents secondary data source catalog, covering ID, Dataset / Corpus, N / Scope and Role.
...
\noindent Table~\ref{tab:secondary_data_outcome_map} presents secondary data outcome map, covering Stage, Output and Chapter~4 finding.
...
\noindent Table~\ref{tab:hybrid_data_reference_catalog} presents hybrid data phase catalog, covering ID, Hybrid integration topic, Governance outcome and TOP-8.
...
\noindent Table~\ref{tab:hybrid_data_outcome_map} presents hybrid data outcome map, covering H-x, Primary contribution and Secondary contribution.
...
\noindent Table~\ref{tab:hypothesis_application_flow} presents hypothesis application flow (h1--h10), covering ID, Hypothesis, Items used, and 2 more.
...
\noindent Table~\ref{tab:primary_pediatric_population} presents primary data --- pediatric asd population, covering Population Attribute, Specification and Rationale / Source.
...
\noindent Table~\ref{tab:primary_clinical_data_items} presents primary data --- required clinical items, covering Field Name, Data Type, Format Expectation and Research Purpose.
...
\noindent Table~\ref{tab:primary_behavioral_assessments} presents primary data --- behavioral assessments, covering Instrument, Status, Score format + date-of-administration and Informs.
...
\noindent Table~\ref{tab:primary_eeg_technical} presents primary data --- eeg technical specifications, covering Specification Field, Minimum / Preferred and Detail / Rationale.
...
\noindent Table~\ref{tab:primary_inclusion_exclusion} presents primary data --- inclusion / exclusion criteria, covering Decision, Criterion and Rationale.
...
\noindent Table~\ref{tab:primary_not_required_identifiers} presents primary data --- identifiers not required, covering Identifier, Status and Rationale for NOT collecting.
...
\noindent Table~\ref{tab:primary_required_documentation} presents primary data --- required documentation, covering Document, Status and Purpose / Acceptance criterion.
...
\noindent Table~\ref{tab:primary_validation_support_requested} presents primary data --- validation + workflow support requested, covering Support Area, Specialist Type Requested and Requested Contribution.
...
\noindent Table~\ref{tab:primary_analysis_preprocessing} presents primary data --- preprocessing diagnostic plan, covering \#, Preprocessing step, Diagnostic computed and Before / after artefact pair.
...
\noindent Table~\ref{tab:primary_analysis_reliability} presents primary data --- reliability analysis plan, covering Reliability type, Instrument / feature, Method + acceptance threshold and Reporting.
...
\noindent Table~\ref{tab:primary_analysis_validity} presents primary data --- convergent + discriminant validity, covering Validity type, Instrument / construct pair, Method + threshold and Expected.
...
\noindent Table~\ref{tab:primary_analysis_eeg_assessment_corr} presents primary data --- eeg--assessment correlation matrix plan, covering EEG feature family, Specific features, Assessment construct paired and Test.
...
\noindent Table~\ref{tab:primary_analysis_subgroup_fairness} presents primary data --- subgroup + fairness analysis plan, covering Subgroup axis, Subgroups compared, Fairness metric + threshold and Action on disparity.
...
\noindent Table~\ref{tab:primary_analysis_missing_data} presents primary data --- missing-data analysis plan, covering Stage, Method, Detail + threshold and Sensitivity check.
...
\noindent Table~\ref{tab:primary_analysis_sensitivity_bias} presents primary data --- sensitivity + bias analysis plan, covering Sensitivity check, Method, Acceptance criterion and Reporting.
...
\noindent Table~\ref{tab:primary_analysis_sap_per_hypothesis} presents primary data --- statistical analysis plan per hypothesis, covering H\#, Hypothesis (paraphrased), Primary test + effect size and Power + sample-size requirement.
...
\noindent Table~\ref{tab:primary_analysis_triangulation} presents primary data --- triangulation plan, covering Evidence type, Source, Convergence test and Convergence threshold.
...
\noindent Table~\ref{tab:research_anatomy} presents research anatomy mapping, covering H\#, Research Gap, Research Objective, and 3 more.
...
\noindent Table~\ref{tab:primary_iv_dv_framework} presents primary data --- iv/dv/rgaig/smart-e per hypothesis, covering H\#, IV (Driver), DV (Outcome), and 2 more.
...
\noindent Table~\ref{tab:hypothesis_impact_scorecard} presents hypothesis impact scorecard, covering H\#, Overall, Time, and 6 more.
...
\noindent Table~\ref{tab:survey_result_catalog} presents survey result catalog, covering Construct / Hypothesis, Threshold, Achieved, and 3 more.
...
\noindent Table~\ref{tab:b2c_research_gap_catalog} presents b2c research gap catalog, covering G\#, B2C Research Gap, Sev. (B2C) and Closing H.
...
\noindent Table~\ref{tab:b2c_survey_result_catalog} presents b2c survey result catalog, covering Hypothesis (B2C), Threshold, Achieved, and 2 more.
...
\noindent Table~\ref{tab:b2c_value_roi_kpi_impact} presents b2c value/roi/kpi/impact scorecard, covering H\#, B2C Value Claim, Caregiver KPI, and 2 more.
...
\noindent Table~\ref{tab:b2b_research_objective_catalog} presents b2b research objective catalog, covering O\#, B2B Objective Statement, Clinician/\allowbreak Hospital Outcome and Score.
...
\noindent Table~\ref{tab:b2b_research_gap_catalog} presents b2b research gap catalog, covering G\#, B2B Research Gap, Sev. (B2B) and Closing H.
...
\noindent Table~\ref{tab:b2b_value_roi_kpi_impact} presents b2b value/roi/kpi/impact scorecard, covering H\#, B2B Value Claim, Clinician/Hospital KPI, and 2 more.
...
\noindent Table~\ref{tab:customer_smart_flow} presents customer + smart analysis flow, covering Step, Customer action, What SMART-E does and SMART-E pillar.
...
\noindent Table~\ref{tab:expected_outputs_map} presents expected outputs map, covering Input / Source, Method, Expected output, and 2 more.
...
\noindent Table~\ref{tab:iv_dv_master_reference} presents master iv/dv reference, covering Variable, Role, P, and 3 more.
...
\noindent Table~\ref{tab:primary_analysis_cards} presents primary data analysis cards a--e, covering Card, Stage, Method, and 2 more.
...
\noindent Table~\ref{tab:primary_analysis_success_metrics} presents primary analysis card success metrics, covering Card, Metric, Threshold, and 2 more.
...
\noindent Table~\ref{tab:secondary_analysis_cards} presents secondary data analysis cards a--f, covering Card, Stage, Method, and 2 more.
...
exemption (45~CFR §46.104(d)(4)) rather than collecting fresh primary
...
\noindent Table~\ref{tab:hybrid_analysis_cards} presents hybrid data analysis cards a--d, covering Card, Stage, Method, and 2 more.
...
\noindent Table~\ref{tab:ch3_analysis_methods_xstream} presents analysis methods applied --- primary / secondary / hybrid, covering Method, Purpose, Primary, and 2 more.
...
\noindent Table~\ref{tab:ch3_monte_carlo_uses} presents monte carlo use-case map --- primary / secondary / hybrid, covering Use case, Stream(s) and What it estimates.
...
\noindent Table~\ref{tab:ch3_todo_status_xstream} presents todo / status map --- primary / secondary / hybrid, covering Task, Primary, Secondary, and 2 more.
...
\noindent Table~\ref{tab:ch3_chapter_summary_xstream} presents chapter summary matrix --- primary / secondary / hybrid, covering Chapter, Primary contribution, Secondary contribution and Hybrid contribution.
...
\noindent Table~\ref{tab:b2c_b2b_assessment_crosswalk} presents b2c/b2b questionnaire $\times$ assessment instrument crosswalk, covering Instrument, Track, Lifecycle Stage, and 2 more.
...
\noindent Table~\ref{tab:data_needs_by_stage_audience} presents data and assessment needs by stage and audience, covering Stage, Audience, Quantitative Data, and 2 more.
...
\noindent Table~\ref{tab:ethical_ai_components} presents ethical ai, covering Ethical Component, Input, Process, and 2 more.
...
\noindent Table~\ref{tab:governance_ai_kpis} presents governance ai, covering KPI, B2C Perspective, B2B Perspective and Audit Perspective.
...
\noindent Table~\ref{tab:trust_ai_ipo_standards} presents trust ai, covering Trust Construct, Input, Process, and 2 more.

{\tablestripes\tablefontsize

\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.2cm} p{2.5cm} p{3.6cm} p{3.0cm} >{\raggedright\arraybackslash}p{2.6cm} @{}}
...
\paragraph{Primary-Data Hypothesis and Decision Logic.}
...
\noindent Table~\ref{tab:research_variables} presents the research variable definitions.
...
\noindent Table~\ref{tab:ch3_sem_model} presents integrated sem path model, covering Path (IV $\to$ DV), Hypothesis, Part, and 2 more.
...
\paragraph{Monte Carlo Simulation Design Parameters.}
\needspace{20\baselineskip}
...
\paragraph{Ten-Phase Framework Master Configuration.}
...
\noindent Table~\ref{tab:10phase_master} presents ten-phase framework master configuration, covering Phase, Name, Objective, and 2 more.
...
\noindent Table~\ref{tab:ai_risk_register} presents the ai evaluation risk register: iso 31000 aligned.
...
\noindent Table~\ref{tab:opdefs} presents operational definitions of core constructs, covering Construct and Operational definition (this dissertation).
...
\noindent Table~\ref{tab:logic_model} presents logic model for the dissertation programme, covering Layer and Content.
...
\noindent Table~\ref{tab:ppi} presents patient and public involvement commitments, covering PPI element and Operational commitment.
...
dimension that these five controls feed). The Ch.5 §5.6 RGAIG section
...
\noindent Table~\ref{tab:dmp} presents data management plan, covering DMP element and Commitment.
...
\noindent Table~\ref{tab:pathway_applies} presents pathway-scope matrix, covering E2E artefact / step, What it covers, Primary data (Phase-2), and 2 more.
...
Every transition writes an audit row per §3.9 governance plan.}
...
\noindent Table~\ref{tab:task_flow_actor} presents per-actor task flow across the seven patient-journey stages, covering Stage, Task, Caregiver, and 3 more.
...
\noindent Table~\ref{tab:trust_cards} presents trust mechanism cards --- promise, covering Promise to participant, Mechanism, Control and Audit artefact.
...
agreement. Each row maps to a §3.9 governance control and an Appendix~M
monitoring rule.
\end{table}
...
described in §3.7.2) rather than MCMC, because (i) the Phase-1 sample
...
\noindent Table~\ref{tab:tech_framework} presents techniques and frameworks used --- with rationale, covering Category, Technique / Framework, Purpose and Section.
...
\endfirsthead
\endhead
Statistical inference & Bootstrap (BCa, 1{,}000 resamples) & Robust CI under small samples & §3.7.1 \\
Statistical inference & Permutation test (Holm correction) & Family-wise error control across H1--H10 & §3.7.2 \\
Sensitivity analysis & Parametric bootstrap (scenario perturbation) & Test stability under noise / drift; replaces Monte Carlo for this study & §3.7.2 \\
Sensitivity analysis & One-at-a-time + tornado plot & Identify dominant input factors & §3.7.3 \\
Causal inference & PLS-SEM (H1--H10 universe) & Test mediation/moderation paths & §3.7.4 \\
Predictive ML & XGBoost, RF, SVM ensemble & Tabular EEG-feature classification & §3.6 \\
Deep learning & 1D-CNN + transformer for EEG & Spectro-temporal pattern learning & §3.6 \\
Generative QA & 220-module GenAI taxonomy & Hallucination + safety eval & §3.6 \\
Explainability & SHAP, LIME, integrated gradients & Per-prediction attribution & §3.7 \\
Trust framework & RGAIG 7-layer + 525-module taxonomy & End-to-end governance & §3.4 \\
Monitoring & Drift (PSI/CSI), fairness (DI/EO-gap), cost & Continuous post-deploy monitoring & §3.8 + Appendix M \\
Compliance gates & HIPAA, GDPR, EU AI Act, FDA SaMD, DPDP, NIST AI RMF & Multi-jurisdictional readiness & §3.9 \\
...
Carlo / MCMC and the choice of BCa bootstrap is justified in §3.7.2;
...
\paragraph{KPI, ROI and value-realisation matrix.}
...
\noindent Table~\ref{tab:kpi_roi_value} presents quality kpi, covering Class, KPI, Dir., and 2 more.
...
\noindent Table~\ref{tab:compliance_crosswalk} presents healthcare-compliance crosswalk, covering Framework, Obligation, Control implemented and Evidence.
...
\endfirsthead
\endhead
HIPAA (US) §164.312 & Encryption + access control + audit log & AES-256 at rest, TLS 1.3 in transit, RBAC, write-once log & Encryption + audit log \\
...
NIST AI RMF (Govern/Map/Measure/Manage) & Lifecycle risk management & RGAIG 7-layer mapped 1:1 to RMF functions & RGAIG--RMF map (§3.9) \\
...
DPDP Act 2023 (India) §9 & Children's data consent + minimisation & Parental consent + de-identification & Consent + de-id log \\
...
US Common Rule 45 CFR §46.104(d)(4) & Secondary-research exemption (Phase-1 only) & Pre-existing de-identified data & IRB exemption letter \\
...
\noindent Table~\ref{tab:how_achieved} presents ``how has this been achieved'' verdict matrix, covering Examiner question, Artefact (in this chapter or referenced) and Phase-1 maturity.
...
\endfirsthead
\endhead
What has been done?            & Figure~\ref{fig:e2e_trust_flow} + §3.6 pipeline & 9/10 \\
How was it done?               & Figure~\ref{fig:patient_journey_e2e} + Table~\ref{tab:task_flow_actor} & 9/10 \\
How does it comply?            & Table~\ref{tab:compliance_crosswalk} + §3.9 & 8/10 \\
How is trust secured?          & Table~\ref{tab:trust_cards} + §3.4 RGAIG & 9/10 \\
...
How is patient data encrypted?         & Figure~\ref{fig:e2e_trust_flow} step 2 + Table~\ref{tab:trust_cards} row 2 + §3.9 DMP & 9/10 \\
How are sensitivity + statistical analyses defended? & §3.7.1--3.7.3 + Table~\ref{tab:tech_framework} & 9/10 \\
Why Monte Carlo is not used    & §3.7.2 non-MCMC disclaimer + BCa bootstrap rationale & 10/10 \\
\bottomrule
\end{xltabular}}
...
\noindent Table~\ref{tab:ch3_decision_summary} presents the methodological decision summary --- 10 key choices and rationale.
