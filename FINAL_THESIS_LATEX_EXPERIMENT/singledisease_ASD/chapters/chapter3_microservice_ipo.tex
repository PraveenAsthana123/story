...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Each service is owned + maintained by a named team per the dissertation-role per-department structure (Ch.3 sec). API contract pinned in OpenAPI 3.0 spec; per-service test verifies the contract per release. Cross-service calls authenticated via OAuth2 client credentials; per-request \texttt{request\_id} propagated end-to-end.
