...
%% per §51 forensic-substrate policy
%% ============================================================================
...
\noindent\emph{Per-iteration forensic changelog of the 2026-05-27 architecture-expansion session. Each row identifies one iteration with its trigger / additions / build state / snapshot path. Provides forensic traceability policy: when something added, what was added, what state the build reached.}
...
\noindent Table~\ref{tab:arch_iteration_changelog} presents per-iteration changelog, covering Iter, Trigger / scope, Additions + key artefacts and Pages (cumulative).

{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{1.0cm} >{\raggedright\arraybackslash}p{7.4cm} p{3.4cm} >{\centering\arraybackslash}p{2.4cm} @{}}
...
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} Per-iteration changelog is the audit trail policy: provides auditable record of WHO (researcher-prompted) + WHEN (per-iteration) + WHAT (additions) + WHERE (artefact files) + VERIFIED state (build + audit). Each iteration's snapshot remains in \texttt{versions/main/pdf/} for forensic reconstruction.
