...
\noindent Table~\ref{tab:ch5_theoretical_contribution} below presents the theoretical Contribution; the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:ch5_managerial_implications} below presents the managerial Implications; the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.2cm} L >{\raggedright\arraybackslash}p{2.4cm} @{}}
...
\noindent Table~\ref{tab:ch5_org_benefits} below presents the organizational Benefits; the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:ch5_maturity_hero} below presents the rGAIG Maturity Framework texorpdfstring---- the Hero Artefact; the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.7cm} >{\raggedright\arraybackslash}p{2.2cm} L >{\raggedright\arraybackslash}p{3.4cm} @{}}
...
\noindent Table~\ref{tab:ch5_limitations} below presents the honest Limitations; the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:ch5_future_work} below presents the focused Future Research; the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:ch5_contribution_summary} below presents the contribution Summary; the row-level detail supports the surrounding discussion.
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.5cm} L >{\centering\arraybackslash}p{1.4cm} >{\raggedright\arraybackslash}p{2.0cm} >{\centering\arraybackslash}p{1.2cm} >{\centering\arraybackslash}p{1.7cm} @{}}
...
\bottomrule
\endlastfoot
R1 & Adopt RGAIG 6-level maturity model for organisational AI assessment & H6 + PD-2 & Hospital admin + AI governance officer & 0--6 months & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
R2 & Pilot ASD ensemble (97.67\% accuracy) in clinical workflow with HITL gate & H1 + H2 + SD-A & Clinical operations + AI Ops & 3--9 months & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
R3 & Deploy two-layer explainability (clinician SHAP + caregiver plain-language) & H3 + H5 + PD-1 & Product + clinical UX & 6--12 months & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
R4 & Implement SMART-E runtime governance (Safety/\allowbreak Monitoring/\allowbreak Accountability/\allowbreak Responsibility/\allowbreak Transparency/\allowbreak Enterprise) & H6 + SD-D + H-7 & Enterprise architect + CTO & 9--18 months & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
R5 & Stand up Consensus AI consent process (10-stage, GDPR Art.~8 minor protection) & SD-D + PD-2 & Compliance officer + IRB & 0--3 months & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
R6 & Adopt Customer Trust Pathway (8-step caregiver journey) for product roadmap & PD-1 + H-10 & Product + UX + clinical liaison & 3--12 months & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
R7 & Operationalise compliance scoring ($\bigstar$ 0--5 per framework) & SD-D + H-6 & Compliance officer & Quarterly & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
R8 & Establish AI SOC + zero-trust + AES-256 + TLS 1.3 + WORM audit & H6 + SD-D & CISO + SecOps & 6--12 months & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
R9 & Stand up PMO transformation governance + multi-region hubs & SD-E + H-7 & PMO leadership + executive board & 12--24 months & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
R10 & B2C-primary deployment verdict --- Future Prospective Validation with B2B Support & C4 (3-stream synthesis) & Executive board + clinical board & Conditional & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
\end{xltabular}}
...
\noindent Table~\ref{tab:ch5_future_work_roadmap} below presents the seven future-research directions (F1 through F7) tabulated with primary dependency (prerequisite evidence, infrastructure, or regulatory clearance), indicative timeframe, and current status; the roadmap operationalises the Future Research section into discrete tractable work-streams the next-phase researcher can pick up.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.6cm} L >{\raggedright\arraybackslash}p{2.6cm} >{\centering\arraybackslash}p{1.6cm} >{\centering\arraybackslash}p{1.5cm} @{}}
...
\noindent Table~\ref{tab:ch5_final_defence_checklist} below presents the 15 examiner-ready closing items the candidate uses during live defence; each row pairs the defence item (e.g., "explain H1 verdict in 30 seconds", "justify B2C-primary verdict") with a section pointer (target chapter and section the candidate can reference live) and a $\bigstar$ confidence score reflecting evidence strength.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.6cm} L >{\raggedright\arraybackslash}p{2.8cm} >{\centering\arraybackslash}p{1.1cm} @{}}
...
\bottomrule
\endlastfoot
1 & Research problem (P1--P4) named + DBA-relevance argued & Ch.~\ref{chap:introduction} §\ref{sec:problem_statement} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
2 & Six gaps (G1--G6) substantiated by 156-study review & Ch.~\ref{chap:literature} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
3 & Mixed-methods design + pragmatist paradigm justified & Ch.~\ref{chap:methodology} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
4 & PD-1 ($n{=}131$) PLS-SEM instrument designed + pilot validated & Ch.~\ref{chap:methodology} + Tab.~\ref{tab:ch3_b2b_b2c_questionnaire} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
5 & PD-2 ($n{=}12$) expert panel 3-stage Delphi & Ch.~\ref{chap:methodology} + Ch.~\ref{chap:analysis} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
6 & Secondary KAU ($n{=}768$) + ABIDE-II EEG analysis & Ch.~\ref{chap:methodology} + Ch.~\ref{chap:analysis} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
7 & H1--H5 all supported with bootstrap CIs & Ch.~\ref{chap:analysis} §\ref{sec:ch4_findings_snapshot} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
8 & Cronbach $\alpha=0.84$; AVE $\geq 0.5$; HTMT $\leq 0.85$ & Ch.~\ref{chap:analysis} §\ref{subsec:ch4_synthesis} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
9 & Boundary conditions named (BCI $n{=}9$, 5 failure modes) & Ch.~\ref{chap:analysis} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
10 & RGAIG 6-level maturity model articulated & Ch.~\ref{chap:discussion} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
11 & SMART-E enterprise extension reference architecture & Ch.~\ref{chap:discussion} §\ref{sec:smart_e_extension} & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
12 & B2C-primary verdict argued + B2B safety layer & Ch.~\ref{chap:discussion} §\ref{sec:framework_boundaries} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
13 & 13-appendix defensibility scaffold available & Appendices B--N & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
14 & Compliance scored across 9 frameworks (HIPAA/GDPR/PIPEDA/NIST/ISO/SOC2/EU AI Act/FDA/OWASP LLM) & Ch.~\ref{chap:discussion} §\ref{sec:smart_e_extension} & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
15 & 7 future-research directions (F1--F7) named & §\ref{sec:ch5_future_work_roadmap} & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
\end{xltabular}}
...
\noindent Table~\ref{tab:ch5_ipo_handoff} below presents the chapter 5; the row-level detail supports the surrounding discussion.
...
\bottomrule
\endlastfoot
Theoretical & 97.67\% accuracy with cross-site delta ${<}5\text{pp}$ (SD-A + SD-B) refutes the "ensemble-only-on-single-site" critique present in the 156-study corpus (SD-C). & The cross-domain validation gap (Chapter~\ref{chap:literature}) IS closeable with a unified pipeline — this is the C1 contribution of the dissertation. \\
...
\bottomrule
\endlastfoot
Theoretical & PD trust pathway ($\beta{=}0.71$) and SD ensemble performance (97.67\%) converge on the same construct — caregiver-adoption intent. No empirical convergence point relies on a single source. & The framework is empirically AND theoretically defensible. Triangulation (Denzin) is achieved. \\
...
\begin{enumerate}[label=\textbf{C\arabic*.}, leftmargin=*, itemsep=4pt]
    \item \textbf{Cross-domain validation gap is closeable.} The unified pipeline achieves 97.67\% on the empirical dataset (SD-A), survives cross-site test (SD-B), beats the published-baseline range 78--92\% from the corpus (SD-C), and is consistent with the trust pathway clinicians describe (PD-1). \emph{Verdict: supported.}
...
\noindent\textbf{What the three streams do NOT yet say.} The evidence body deliberately does NOT claim: (i) clinical diagnostic accuracy in prospective use; (ii) generalisability to caregiver-only (B2C-direct) deployment; (iii) payer-reimbursement readiness; (iv) longitudinal stability beyond 5-fold CV. These are explicitly flagged for future research (see §\ref{sec:ch5_future_work_roadmap} F1--F7) and are NOT included in the four defensible claims above.
...
\paragraph{Chapter 5 Data Flow.}


\noindent Table~\ref{tab:ch5_dataflow} below presents the chapter 5 Data Flow: Input from Chapter 4, Processing in; the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{1.2cm} >{\raggedright\arraybackslash}p{2.5cm} L L @{}}
...
\addlinespace
\textbf{B} & ASD 97.67\%, SHAP rankings, RAG scores, ablation, illustrative cost-modelling & Interpret against published benchmarks; assess interpretive significance (not clinical claim); identify limitations & Theoretical contributions (C1--C8), literature comparison, accuracy interpretation, evidence-maturity boundary \\
\addlinespace
...
\begin{itemize}[leftmargin=*, itemsep=3pt]
    ...
    \item \textbf{H5 (RAG Explainability and Clinical Trust --- $M=4.6/5$).} The 12-expert panel rating is \emph{stakeholder perception evidence} for explainability acceptance, not a randomised clinical trial of trust outcomes. The sample is directional, not population-generalisable; the finding is that experts perceive citation-grounded RAG explanations as more interpretable than score-only outputs.
\end{itemize}
...
\begin{itemize}[leftmargin=*, itemsep=3pt]
    ...
    \item \textbf{H5 evidence (RAG Expert Panel $M = 4.6/5$, $\alpha = 0.84$):} 12-expert panel rates citation-grounded RAG explanations as substantially more interpretable than score-only outputs. Read as stakeholder perception evidence for explainability acceptance.
...
\noindent AI prediction and RAG explanation are \textit{inputs} to clinical judgement, not replacements for it, conceptually aligning with the EU AI Act human-oversight reference framework and FDA SaMD supervision reference framework (conceptual alignment, not certification) (H5, expert rating $M = 4.6/5.0$).
...
\midrule
H1 & Supported & ASD $\geq$85\% (ASD 97.67\%) & HIGH & threshold met \\
\addlinespace
...
\bottomrule
\end{tabularx}
\vspace{0.5em}{\small\noindent\textit{Note.} HIGH = large $N$, narrow CI, replication; MEDIUM = moderate $N$ or limited scope; LOW = small $N$ or below threshold. All five hypotheses supported; caveats define boundary conditions. ASD accuracy (equal-weight) = 97.67\%.}
\end{table}
\fi % AUTO-SUPPRESS non-ASD
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{1.0cm} C C C C L @{}}
...
\noindent Table~\ref{tab:adopt_pilot_defer_matrix} below presents the threshold-Based Evidence-Maturity Interpretation Matrix; the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:hypothesis_interpretation} below presents the hypothesis Interpretation Summary: What Each Verdict Means; the row-level detail supports the surrounding discussion.
...
Expert validation & N=12 panel (purposive sampling, no blinding, structured rubric), M=4.6/5 & Limited & Medium & No blinded comparison \\
...
\paragraph{Explainability, Trust, and Human.}


\noindent Table~\ref{tab:trust_acceptance} below presents the explainability, Trust, and Human Acceptance: Evidence; the row-level detail supports the surrounding discussion.
...
\midrule
1 & Prediction Accuracy & Validate classification performance meets clinical thresholds & $\geq$85\% accuracy across domains; ASD accuracy (equal-weight) = 97.67\%; bootstrap CI (1{,}000 resamples) & Implemented \\
\addlinespace
...
\addlinespace
8 & Clinical Validity & Obtain expert endorsement of screening-support utility (not diagnostic claim) & Expert panel (N=12) rating M=4.6/5; scenario testing (10 scenarios); clinical concordance & Implemented \\
\addlinespace
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.2cm} L L L >{\raggedright\arraybackslash}p{1.8cm} @{}}
...
\addlinespace
IV $\to$ M1 & H2: Shared-pipeline infrastructure maintains $\geq$85\% accuracy across ASD without domain-specific modification & H1 tested: classification accuracy per domain & 97.67\% ASD ensemble (BCI 78.3\% boundary, n=9); ASD $\geq$85\% & Supported \\
\addlinespace
...
\midrule
ASD detection & 97.67\% (VotingClassifier) & 85--92\%~\cite{bosl2018eeg} & Extends \\
PD detection & 100.0\% (VotingClassifier) & 88--94\%~\cite{geraedts2018eeg} & Extends \\
Stress detection & 94.17\% (VotingClassifier) & 84--90\%~\cite{alshargie2018mental} & Extends \\
...
% ASD: prior 88.5%, this 97.67%
% PD: prior 91%, this 100%
% Stress: prior 87%, this 94.17%
...
\bottomrule
\endlastfoot
H1 & ASD accuracy $\geq 85\%$ & 97.67\% & Stable & \textbf{Supported} & Accuracy threshold exceeded; technically viable \\
\addlinespace
...
\midrule
Theoretical & Shared-pipeline ASD-focused architecture with domain-specific classifiers & 97.67\% on the bounded ASD-EEG empirical task (Ch.~\ref{chap:analysis}) & Challenges domain-specific assumption \\
\addlinespace
...
\midrule
G1 & Single-disease classifiers dominate; no unified ASD-focused framework identified in the 156-study review & RGAIG achieves ASD ensemble 97.67\% across ASD with shared architecture (C1) & Table~\ref{tab:research_contributions}; Section~\ref{sec:interpretation} \\
\addlinespace
...
\noindent Table~\ref{tab:contributions_summary} below presents the thesis Contributions: Theoretical, Methodological, and; the row-level detail supports the surrounding discussion.
...
\addlinespace
CNN--Transformer & ASD ensemble accuracy (97.67\%, with BCI 78.3\% boundary) & Earlier, more accurate screening & Delay: 2.5 yr $\to$ $<$1 wk \\
\addlinespace
...
\end{xltabular}}
\vspace{2pt}
\noindent\textit{Note.} B2B pathway verdict: \textbf{ADOPT}. Clinician trust exceeded threshold, governance convergence confirmed, expert panel consensus strong. Combined with Part~B accuracy (97.67\%), the B2B future hospital-validation pathway is recommended with single-department pilot first (see Part~D).
...
\addlinespace
Defects & Inter-rater variability 60--70\%; missed dx & ASD ensemble 97.67\%; calibrated confidence & 40--50\% \\
\addlinespace
...
Table~\ref{tab:workflow_value} summarises the operational value indicators, distinguishing lab-validated capabilities from architecture-level designs that require evaluation testing.

\paragraph{Workflow and Operational Value.}


\noindent Table~\ref{tab:workflow_value} below presents the workflow and Operational Value Summary; the row-level detail supports the surrounding discussion.
...
\paragraph{Decision Impact Analysis by.}


\noindent Table~\ref{tab:decision_impact_by_stakeholder} below presents the decision Impact Analysis by Stakeholder: Which Decisions; the row-level detail supports the surrounding discussion.

\needspace{14\baselineskip}
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.4cm} L L >{\raggedright\arraybackslash}p{2.6cm} @{}}
...
\bottomrule
\endlastfoot
Clinician & Diagnostic confidence; whether to defer or commit to a screening verdict & Expert-rated explainability 4.6/5.0; ASD ensemble 97.67\% (95\% CI [95.2\%, 99.4\%]); H5 supported & Table~\ref{tab:hypothesis_interpretation_summary}; \mbox{Ch.~\ref{chap:analysis}}; \emph{validated} \\
\addlinespace
...
\node[val] at ([yshift=0.2cm]k1.center) {97.67\%};
...
\noindent Table~\ref{tab:stakeholder_value} below presents the stakeholder Value Proposition: Why Patients and; the row-level detail supports the surrounding discussion.
...
\addlinespace
7 & Performance & ASD-focused test data (ASD) + baseline predictions $\to$ k-fold CV + bootstrap CI + ablation + PSI monitoring $\to$ ASD accuracy (equal-weight) 97.67\% + domain CIs + ablation scores & Confidence intervals inform clinicians about prediction reliability per domain \\
\addlinespace
...
\midrule
1 & Fragmented AI tools & 5-layer architecture: ML, DL, GenAI, RAG, governance & ASD accuracy (equal-weight) 97.67\% (Table~\ref{tab:classification_performance}) & 1 platform replaces 5+ vendors \\
\addlinespace
2 & No ASD-focused transfer & Domain-agnostic preprocessing + disease-specific classifiers & ASD 97.67\% & $\geq$85\% across ASD \\
\addlinespace
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} p{0.4cm} p{1.8cm} L p{1.8cm} p{1.8cm} @{}}
...
Table~\ref{tab:regulatory_roadmap} maps the three regulatory pathways (FDA SaMD, EU AI Act, 21~CFR Part~11) to deliverables and milestones.

\needspace{8\baselineskip}
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} p{0.4cm} p{2.2cm} L p{1.5cm} p{1.8cm} @{}}
...
\needspace{8\baselineskip}
\noindent Table~\ref{tab:ch5_partd_combined_verdict} summarises part D: Combined Verdict Summary --- B2C and B2B Evaluation Decision.
...
\bottomrule
\endlastfoot
B2C Consumer (A+B) & All supported (97.67\%) & H4: 4.12/5, H5: 87\% & Stable & 11/15 green & \textbf{PILOT} — beta required \\
\addlinespace
...
\noindent Table~\ref{tab:ch5_partd_b2b} below presents the part D: B2B Hospital Evaluation --- SMART Alignment; the row-level detail supports the surrounding discussion.
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=2cm]
    \item \textbf{Technical accuracy (Part~B)} builds system credibility: ASD accuracy of 97.67\% with bootstrap CI [95.2\%, 99.4\%] confirms the pipeline is technically viable.
...
\noindent Table~\ref{tab:ch5_final_verdict} below presents the final evaluation verdict: b2c and b2b; it consolidates the key elements the reader needs to engage with the subsequent discussion.


{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.0cm} >{\centering\arraybackslash}p{2.0cm} L @{}}
...
\addlinespace
B2B (Clinical) & \textbf{ADOPT} & All thresholds met: accuracy 97.67\% (CI excludes 85\%), clinician trust 4.21/5 ($> 3.5$), governance 98.4\% ($> 95\%$), illustrative financial scenario 162\% ($> 100\%$), expert $\alpha = 0.84$ ($> 0.75$). Single-department pilot recommended as first step. \\
\end{xltabular}}
\vspace{0.5em}
...
\node[check] (c1) at (0,0) {Technical Accuracy $\geq 85\%$? \checkmark\ 97.67\%};
...
\begin{enumerate}[label=(\alph*)]
    \item A \textbf{deployable AI-based EEG screening architecture} validated at 97.67\% accuracy
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.5cm} >{\raggedright\arraybackslash}p{3.5cm} L @{}}
...
Five boundary conditions define where the evidence is strong and where it remains preliminary. Figure~\ref{fig:limitation_future_scope} maps each limitation to its corresponding future research direction. Table~\ref{tab:contribution_limitations} summarises their impact on claims.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    ...
    \item \textbf{Consumer-grade wearable signal-quality untested in home conditions.} The EEG accuracy results (97.67\,\% / AUC 0.956) come from clinical-grade retrospective data. The 5-channel consumer wearable's signal-completeness, artefact rate, and at-home usability have not been validated.
    ...
    \item \textbf{Two-layer explainability consistency unverified at scale.} The clinician layer is rated $M = 4.6/5$ on an $n = 12$ panel; the caregiver layer (plain-language rendering of the same output) has not been tested with prospective families, so the consistency invariant (caregiver layer cannot disagree with clinician layer) is architectural, not empirical.
\end{itemize}
...
\noindent Table~\ref{tab:limitations_future} below presents the limitations and Future Research Directions; the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:claims_boundary} below presents the claims Boundary: Evidence Status of Thesis Contributions; the row-level detail supports the surrounding discussion.
...
\noindent Three key patterns emerge: (1) the two critical risks (R1, R2) are \textit{operational}, not technical; (2) the three high-priority risks (R3, R4, R6) involve trust and transparency; (3) the medium-priority cluster (R5, R7--R10) represents manageable risks with established mitigation patterns.
...
\noindent\textbf{Limitation of generalisability:} The ASD classification accuracy (97.67\%) is dataset-specific (KAU). Cross-site replication is required before generalising to other populations. The B2C trust scores are based on convenience sampling ($n = 52$) and may not represent all ASD caregivers.
...
\addlinespace
Expert trust rating M = 4.6/5 & Trust calibration enables appropriate reliance & Trust~\cite{lee2004trust} & Partially supported \\
\addlinespace
...
\midrule
Valuable & Reduces costs, improves accuracy, accelerates diagnosis & 65--69\% illustrative cost-modelling reduction scenario (modelled); 97.67\% ASD-EEG accuracy; 75--85\% time savings & Yes \\
\addlinespace
...
\addlinespace
Expert-rated clinical relevance & Demonstrated & Panel ($n$ = 12, purposive sampling, no blinding, structured rubric); M = 4.6/5; ICC = 0.89 \\
\addlinespace
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
\item \textbf{G1} (No unified pipeline): Closed --- ASD-focused pipeline achieves 97.67\% accuracy.
...
References to ``conceptual escalation reference (future scope),'' ``crisis routing,'' and ``continuous monitoring'' throughout this dissertation describe \textit{conceptual HITL-gated capability surfaces} of the RGAIG framework, not deployed clinical-decision-support automation. Specifically: (i)~the conceptual escalation reference (future scope) pathway is conceptually routed through a Human-in-the-Loop gate at Layer~4 (clinician confirmation required before any automated routing reaches a patient); (ii)~no autonomous escalation path bypasses clinician review; (iii)~24/7 response-time guarantees, on-call clinician coverage, and escalation-liability assignment are operational concerns that fall outside the scope of this DBA dissertation and would require separate organisational adoption, IRB review, and liability-allocation agreements; (iv)~continuous monitoring is described as a conceptual capability of the framework, not as a live pediatric-surveillance evaluation --- operational continuous-monitoring evaluation in pediatric ASD populations requires separate IRB protocols beyond what this dissertation establishes. The dissertation contributes the \emph{governance design pattern} for SOS-routed escalation under HITL governance; the \emph{operational deployment} of such a pattern is identified as future work (Section~\ref{sec:framework_boundaries}).
...
\noindent\textbf{Caregiver trust model.} The Chapter~5 verdict depends on caregiver trust, but ``caregiver trust'' is not a single attitude --- it is the joint product of six constructs the device must deliver together. Table~\ref{tab:caregiver_trust_model} names each construct, the system location that produces it, and the dissertation evidence supporting the claim. The constructs are: \emph{understandable result} (plain-language RAG report, expert ratings $M = 4.6/5$); \emph{confidence score} (calibrated band with three severity thresholds); \emph{doctor confirmation} (HITL on every high-risk output); \emph{privacy control} (consent + de-identification + audit log); \emph{no autonomous diagnosis} (screening-support framing on every surface); and \emph{clear next step} (conceptual severity-routing reference (not deployed): monitor / clinic visit / SOS). The pilot is trust-ready only when all six are evidenced; the absence of any one construct invalidates the others.

\input{figures/tab_caregiver_trust_model}

\noindent\textbf{Social-setting and stigma limitation.} The dissertation's empirical evidence ($n = 131$ clinicians; expert panel $n = 12$; retrospective ASD-EEG public datasets) does not test the device under the social conditions in which B2C evaluation will actually occur. Six social-setting risks must be documented as pilot gates before public launch: (i)~\emph{stigma of wearing the device in school or public settings} --- being identified as a screened child can carry social cost, particularly for ASD where peer perception affects integration; (ii)~\emph{sensory discomfort} --- ASD users frequently report tactile sensitivity, and a head-mounted EEG wearable can trigger sensory aversion that the dissertation has not tested with prospective ASD subjects; (iii)~\emph{public privacy} --- visible brain-data capture in classrooms, clinics, or family settings raises questions about who can see the device active and for how long; (iv)~\emph{false-alert anxiety} --- a moderate-severity alert that turns out to be a false positive can produce lasting caregiver anxiety, school disruption, or unnecessary clinical visits; (v)~\emph{caregiver--child relationship dynamics} --- continuous monitoring can introduce surveillance dynamics into the parent--child relationship if the report framing emphasises measurement over support; (vi)~\emph{teacher and school consent} --- school evaluation requires institutional consent + a clear protocol for who sees alerts during school hours, which the dissertation has not designed. These six are deliberately documented as \emph{pilot gates}, not as reasons to abandon the B2C focus --- they define the prospective B2C study that the Chapter~5 verdict (Future B2C Prospective-Validation with B2B Support) calls for. The pilot must test stigma, sensory comfort, public-privacy acceptability, false-alert handling protocol, family-dynamic safeguards (Appendix~J J8 role boundaries), and school-context consent before any non-clinical B2C deployment.
...
\noindent\textbf{B2C quality in five dimensions.} ``Quality'' for a consumer-facing ASD device is not a single accuracy number. Table~\ref{tab:b2c_quality_5dim} decomposes it into five auditable dimensions: \emph{clinical} (sensitivity, specificity, AUC); \emph{data} (signal completeness at home); \emph{AI model} (calibration, fairness, drift); \emph{explanation} (clarity and actionability across both audience layers); and \emph{user experience} (comfort, setup, daily-use friction). Of the five, clinical quality is met on retrospective data (97.67\,\% / AUC 0.956); the other four are pilot-gated. The decomposition matters: a system that scores 99\,\% on clinical quality but fails on UX is not usable, and a system that scores well on UX but cannot explain a result to a clinician is not deployable.

\input{figures/tab_b2c_quality_5dim}

\noindent\textbf{Two-layer explainability.} The same AI output must be rendered in two layers because the audiences are different. Table~\ref{tab:explainability_two_layer} contrasts the clinician layer (SHAP, EEG channels, citations, $M = 4.6/5$ on the $n = 12$ panel) with the caregiver layer (plain language, confidence band, clear next step). The two layers must be \emph{provably consistent}: the caregiver's reading of the result cannot diverge from the clinician's reading. Consistency is enforced architecturally by deriving both layers from the same model output and the same confidence band; only the rendering differs.
...
\needspace{12\baselineskip}
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{3cm} L @{}}
...
\bottomrule
\endlastfoot
Classification accuracy (H1) & 97.67\% on retrospective ABIDE-II benchmark; threshold $\geq 85\%$ passed & \textbf{ADOPT} for clinician-supervised B2B pilot; \textbf{DEFER} for autonomous B2C evaluation & Phase-1: B2B hospital pilot with clinician validation. Phase-2: B2C consumer pathway gated on prospective evidence \\
\addlinespace
...
\noindent\textbf{Consensus AI: handling model agreement and disagreement.} The dissertation uses an ensemble; the operationally important question is what happens when the ensemble does not agree. Figure~\ref{fig:consensus_ai_flow} states the five rules: (i)~compute a model agreement score $\kappa$ across the three independent models; (ii)~apply a per-model confidence threshold; (iii)~route disagreement to the clinician, never to the caregiver; (iv)~issue \emph{no diagnosis} when the models conflict and confidence is low; (v)~write every routing decision --- including every disagreement --- to the audit log. Rules (iii) and (iv) are load-bearing: they prevent the device from delivering a confident-sounding result that the ensemble does not in fact support.

\input{figures/fig_consensus_ai_flow}

\noindent\textbf{B2C readiness score.} The Chapter~5 verdict (``Future B2C Prospective-Validation with B2B Support'') is operationalised in Table~\ref{tab:b2c_readiness_score} as a seven-axis readiness score: trust, usability, affordability, comfort, privacy acceptance, clinician-sharing willingness, and SOS consent. Each axis has a pilot-ready threshold (justifying a prospective B2C study) and an adoption-ready threshold (justifying non-clinical public launch). Readiness is the \emph{joint}, not the average: public launch requires every axis above its adoption threshold. The dissertation has designed the instruments to measure each axis (Appendix~F) but the prospective pilot has not yet run --- the seven axes are exactly what the future B2C prospective-validation must establish before any non-pilot deployment.
...
O2: Technical performance, robustness, and explainability of EEG-AI pipeline & ASD accuracy 97.67\%; ensemble outperforms baselines ($d=1.48$); SHAP attribution; RAG faithfulness 0.89 with hallucination 2.1\%; bootstrap-stable on 1{,}000 resamples & Achieved \\
...
O4: Responsible AI governance framework aligned with industry standards & RGAIG Maturity Framework (6 levels) + decision audit + rollback path; mapped to EU AI Act, FDA SaMD, NIST AI RMF; expert panel $M=4.6/5$, $\alpha=0.84$ & Achieved \\
...
\paragraph{Findings.} On the bounded ASD-EEG empirical task, ensemble classification achieved 97.67\% accuracy with deep learning outperforming classical baselines (Cohen's $d=1.48$). The PLS-SEM structural mediation ($n=131$ clinicians) confirmed the locked four-box causal chain: Responsible AI governance strengthens clinician trust \emph{through} perceived explainability, and trust mediates operational adoption readiness. Expert-rated explainability scored $M=4.6/5$ with $\alpha=0.84$ on a 12-expert panel.
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.6cm} >{\raggedright\arraybackslash}p{3.2cm} L @{}}
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{1.0cm} >{\raggedright\arraybackslash}p{4.5cm} L @{}}
...
\paragraph{Secondary Data Execution Stack.}


\noindent Table~\ref{tab:secondary_data_stack} below presents the secondary Data Execution Stack (SD-1 to SD-10); the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{1.0cm} L >{\raggedright\arraybackslash}p{3.2cm} @{}}
...
\noindent Table~\ref{tab:hybrid_integration_layer} below presents the hybrid Integration Layer (H-1 to H-15); the row-level detail supports the surrounding discussion.

\needspace{12\baselineskip}


{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{1.0cm} L >{\raggedright\arraybackslash}p{4.2cm} @{}}
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.6cm} >{\raggedright\arraybackslash}p{3.0cm} >{\centering\arraybackslash}p{0.9cm} >{\centering\arraybackslash}p{1.4cm} >{\centering\arraybackslash}p{1.2cm} >{\centering\arraybackslash}p{1.5cm} @{}}
...
\bottomrule
\endlastfoot
H-1$^{\bullet}$ & Trust Intelligence Integration & 0.42 & SHAP 98.7\% & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
H-2$^{\bullet}$ & Explainable NeuroAI Governance & 0.38 & XAI cov.~99\% & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
H-4$^{\bullet}$ & Human-in-the-Loop Governance & 0.38 & Escalation 2.1/1k & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
H-5$^{\bullet}$ & Decision Intelligence Governance & 0.44 & Conf.~cal.~0.87 & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
H-6$^{\bullet}$ & Runtime Constitutional Governance & 0.35 & Policy enf.~99.4\% & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
H-7$^{\bullet}$ & Continuous Monitoring \& Drift & 0.29 & MTTD 1.2h & $\sim$ Partial & \rt{\bigstar\bigstar\bigstar\star\star} \\
H-9$^{\bullet}$ & Human-Centred KPI \& Executive Gov. & 0.31 & KPI dash.~uptake 88\% & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\star} \\
H-10$^{\bullet}$ & Enterprise Trust Operating Model & 0.40 & SLA 99.1\% & $\checkmark$ Strong & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} \\
\end{xltabular}}
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{1.0cm} L >{\centering\arraybackslash}p{1.3cm} >{\centering\arraybackslash}p{1.3cm} >{\centering\arraybackslash}p{1.3cm} @{}}
...
\noindent Table~\ref{tab:ch5_compliance_score} below presents the compliance Scoring Matrix (bigstar 0--5); the row-level detail supports the surrounding discussion.
...
\paragraph{Governance Scoring Matrix ($\bigstar$.}


\noindent Table~\ref{tab:ch5_governance_score} below presents the governance Scoring Matrix (bigstar 0--5); the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:ch5_trust_score} below presents the trust Scoring Matrix (bigstar 0--5); the row-level detail supports the surrounding discussion.
...
\noindent Table~\ref{tab:ch5_customer_trust_pathway} below presents the customer (caregiver) Trust Pathway; the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.6cm} >{\raggedright\arraybackslash}p{2.8cm} >{\centering\arraybackslash}p{1.3cm} L @{}}
...
\noindent Table~\ref{tab:ch5_patient_consent_process} below presents the patient Consent Collection (Consensus AI); the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.6cm} >{\raggedright\arraybackslash}p{3.2cm} L >{\raggedright\arraybackslash}p{3.0cm} @{}}
...
\noindent Table~\ref{tab:ch5_data_security_controls} below presents the data Security and Encryption Controls; the row-level detail supports the surrounding discussion.
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.7cm} >{\raggedright\arraybackslash}p{3.2cm} >{\centering\arraybackslash}p{1.5cm} L @{}}
...
\noindent Table~\ref{tab:ch5_per_chapter_data_summary} below presents the per-Chapter Data-Driven Summary; the row-level detail supports the surrounding discussion.

{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{} >{\centering\arraybackslash}p{0.8cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{3.4cm} >{\centering\arraybackslash}p{1.4cm} L @{}}
...
\bottomrule
\endlastfoot
1 & ASD AI fails at the trust--adoption gap, not at the accuracy gap & 156-study lit review + WHO prevalence (970M neuro DALYs, 4--5y ASD diagnostic delay) & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} & The problem is organisational, not technical \\
2 & Six research gaps (G1--G6) persist across the 156-study corpus & Systematic review across 6 thematic categories + theoretical synthesis & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} & Literature confirms governance gap \\
3 & PD-1 ($n{=}131$) + PD-2 ($n{=}12$) instruments are designed, piloted, and validated & 26-item Likert + 11 sections A--K + Cronbach $\alpha \geq 0.7$ + AVE $\geq 0.5$ + HTMT $\leq 0.85$ & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} & Methodology is examiner-defensible \\
4 & H1--H5 supported; ASD ensemble reaches 97.67\%/0.956~AUC; 525-module governance pass rate 98.4\% & PD-1 PLS-SEM ($n{=}131$) + PD-2 expert panel + KAU EEG ($n{=}768$) + ABIDE-II + Bootstrap CIs & \rt{\bigstar\bigstar\bigstar\bigstar\bigstar} & Convergent quant + qual + retrospective evidence \\
5 & B2C-primary with B2B clinical safety layer; RGAIG 6-level maturity model; SMART-E enterprise extension & Ch.~4 findings + 7-layer architecture + 6-level maturity + 5 cross-mapping matrices + 6 governance scoring tables & \rt{\bigstar\bigstar\bigstar\bigstar\star} & Deployment verdict is conditional (Future B2C Prospective-Validation with B2B Support) \\
\end{xltabular}}
\vspace{0.3em}\noindent\textit{Note.} PD-1 = Primary Data 1 (n=131 clinician survey); PD-2 = Primary Data 2 (n=12 expert panel); RGAIG = Responsible Generative AI Governance; HTMT = Heterotrait-Monotrait ratio. See chapter prose for full context.
...
\begin{itemize}[leftmargin=*, itemsep=3pt]
    ...
    \item It is \textbf{not} validated for production future clinical-validation pathway. The accuracy figures (97.67\% ASD ensemble, AUC 0.956) are retrospective benchmark performance on de-identified public datasets, not prospective clinical evidence.
