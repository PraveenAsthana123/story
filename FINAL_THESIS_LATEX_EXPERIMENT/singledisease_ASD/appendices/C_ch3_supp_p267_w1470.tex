...
\paragraph{EEG Feature Extraction.}
...
\noindent This table lists each \textit{row} across process, technology, and verification, so examiners can trace what was assessed and what evidence supports each row.
...
\noindent This table lists each \textit{row} across pillar, mod., core question and key analyses, and 1 more dimension, so examiners can trace what was assessed and what evidence supports each row.
...
% 2026-05-30 fix: previous bend left=70 placed the 'feedback to governance'
% label directly on top of the 'Audit log + traceability' (g3) box. Route the
% dashed arrow far to the right via out=0/in=0/looseness=3, and place the
% label outside the box column with align=left + two-line break.
\draw[arr, dashed] (s5.east) to[out=0, in=0, looseness=3]
    node[right=8pt, midway, font=\scriptsize, align=left] {feedback\\to governance}
    (s2.east);
\end{tikzpicture}
...
{\tablestripes\tablefontsize
\needspace{20\baselineskip}
\begin{xltabular}{\textwidth}{@{} >{\raggedright\arraybackslash}p{3.7cm} p{8.5cm} >{\raggedright\arraybackslash}p{2.1cm} @{}}
...
\paragraph{Chapter-Wise Decision Role.}
...
\paragraph{Domain-Sensitive Evaluation Decision.}
...
{\tablestripes\tablefontsize
\begin{xltabular}{\textwidth}{@{\extracolsep{\fill}} >{\raggedright\arraybackslash}p{3.7cm} >{\raggedright\arraybackslash}p{2.7cm} p{5.5cm} >{\centering\arraybackslash}p{1.0cm} >{\raggedright\arraybackslash}p{1.0cm} @{}}
...
\paragraph{EEG Preprocessing Parameter.}
