...
\noindent\textbf{Reference implementation scope.} The reference implementation is provided solely to demonstrate conceptual feasibility and traceability between governance requirements and technical implementation. It is not intended to represent a production-ready software system and does not constitute the primary contribution of the research.
...
Error handling & Per-phase \texttt{try/except} with graceful degradation + audit-log of error + escalation per \\
...
\noindent This table lists each \textit{row} across function name, input type, output type, and 1 more dimension, so examiners can trace what was assessed and what evidence supports each row.
...
Error handling & Per-phase \texttt{try/except} captures exception, writes audit row, escalates per \\
